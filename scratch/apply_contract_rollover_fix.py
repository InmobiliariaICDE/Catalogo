  ...
  if (data && data.properties) {
    ...
    data.properties.forEach(p => {
      ...
      if (p.payments) {
        ...
        if (startDt && (hasTenant || startDt || hasAnyPayment) && !isVacantByName) {
        ...
        chronologicalMonths.forEach((item, idx) => {
          ...
          if (isPaidCell) {
            ...
            m.status = 'DELIVERY';
          } else if (lastDelivery > lastPaid || isBeforeStart || isAfterEnd || isVacantByName || (!hasTenant && !startDt && !hasAnyPayment)) {
            m.status = 'VACANT';
            m.value = 'DESOCUPADO';
          } else if (isFuture) {
            m.status = 'FUTURE';
...
print("Done applying contract rollover fix!")
