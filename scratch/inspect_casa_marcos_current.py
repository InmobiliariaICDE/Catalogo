...
for p in data['properties']:
    if 'CASA MARCOS' in p.get('name', '').upper() or p.get('excel_row') in [10, 11]:
        print(f"Row {p.get('excel_row')} | ID {p.get('id')} | Name: '{p.get('name')}' | Tenant: '{p.get('tenant_name')}' | Start: '{p.get('start_date')}' | Dur: '{p.get('duration')}' | Rent: '{p.get('monthly_rent')}'")
