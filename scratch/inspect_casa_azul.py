...
for p in data['properties']:
    if 'CASA AZUL' in p.get('name', '').upper() or p.get('excel_row') == 18:
        print(f"Row {p.get('excel_row')} | ID {p.get('id')} | Name: '{p.get('name')}' | Tenant: '{p.get('tenant_name')}' | Start: '{p.get('start_date')}' | Dur: '{p.get('duration')}'")
