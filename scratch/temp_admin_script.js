...
function contRenderInversiones(el){
  ...
  if (countSelected > 0) {
    ...
    seleccionResumenHtml = `
      <div style="background:rgba(34,197,94,0.08); border:1px solid rgba(34,197,94,0.3); border-radius:10px; padding:8px 16px; display:flex; align-items:center; gap:14px; flex-wrap:wrap; margin-top:12px; box-shadow:inset 0 1px 0 rgba(255,255,255,0.05); width:100%;">
  ...
  const allIdsJsonEscaped = JSON.stringify(allInversiones.map(x => x.id)).replace(/"/g, '&quot;');
      ...
      <tbody>
        ${allInversiones.map(inv => {
          ...
          <tr style="${rowBgStyle} cursor:pointer;" onclick="contAbrirModalInversion('${inv.id}')" onmouseover="this.style.background='${isSelected ? 'rgba(212,168,75,0.15)' : 'rgba(255,255,255,0.025)'}'" onmouseout="this.style.background='${isSelected ? 'rgba(212,168,75,0.09)' : ''}'">
            ...
            <td style="padding:10px 14px; color:#aaa; max-width:380px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${(inv.accion || inv.desc || '').replace(/"/g, '&quot;')}">
              ${inv.accion || inv.desc || '—'}
            </td>
