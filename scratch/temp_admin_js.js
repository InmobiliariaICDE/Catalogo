      ...
      <div id="mapaContenedor_${pfx}" style="position: relative;">
        ...
        <div class="map-floating-controls" style="position: absolute; top: 10px; right: 10px; z-index: 1000; display: flex; flex-direction: column; gap: 8px; pointer-events: none;">
          ...
          <div class="map-control-card" style="background: var(--bg3); border: 1px solid var(--border); border-radius: 10px; padding: 8px; pointer-events: auto; box-shadow: var(--shadow); display: flex; gap: 8px; align-items: center; justify-content: center;">
            <button class="btn-map-control btn-map-add-mode" id="btnAddProp_nl" onclick="toggleMapAddMode('nl')" title="Agregar propiedad en el mapa">➕</button>
            ...
            <button class="btn-map-control" id="btnGeoMasivo" onclick="abrirGeocodificadorMasivo()" title="Asignar coordenadas a propiedades sin ubicación" style="background:rgba(190,137,57,.15);border-color:rgba(190,137,57,.5);color:#be8939;font-size:10px;padding:0 8px;height:32px;white-space:nowrap;">📍 Sin coords</button>
          </div>
        </div>
      </div>
    </div>`;
...
function normalizarPropiedades(data) {
  ...
  return data.map((obj, i) => {
    ...
    for (let key in obj) {
      ...
      else if (nk === 'direccion' || nk === 'dirección' || nk === 'direccion-vereda' || nk === 'direccion_vereda') k = 'DIRECCIÓN';
    ...
    <div class="citas-layout-nuevo" style="display:grid; grid-template-columns: 320px 1fr; gap:24px; align-items:start;">
      ...
      <div id="listContainer">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; gap: 15px; flex-wrap: wrap;">
          ...
          <div style="display:flex; align-items:center; gap:12px; flex-grow:1; justify-content: flex-end;">
            <div class="pc-buscar-row-premium">
              <span class="bn-icon" style="padding:0 8px 0 12px; color: var(--gold);">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              </span>
              <input type="text" id="searchCitasInput" placeholder="Buscar por cliente o celular..." value="${searchTermCitas}" oninput="buscarCitas(this.value)" style="font-size:13px; background: transparent; color: #fff; border: none; outline: none; width: 100%;"/>
            ...
            <button class="btn-nueva-cita-premium" onclick="abrirVisitaModal()">
...
function abrirEditorProp(codigo, lat, lng) {
  ...
  document.getElementById('edPropCelulares').value = p ? (p['Celulares'] || '') : '';
...
function geoMostrarActual() {
  ...
  document.getElementById('geoLatVal').textContent = '—';
  document.getElementById('geoLngVal').textContent = '—';
...
function autofillPropiedadPorCodigo(selectedCode) {
  ...
  setVal('gp_celulares', p['Celulares'] || p['celulares'] || '');
  setVal('gp_propietario', p['Nombre del Propietario'] || p['propietario'] || p['Nombre del propietario'] || '');
...
async function ejecutarGuardadoPropiedad(prop) {
  ...
  try {
    const scriptUrl = CRM_SCRIPT_URL;
