...
function normalizarPropiedades(data) {
  ...
  return data.map((obj, i) => {
    ...
    for (let key in obj) {
      ...
      else if (nk === 'area lote' || nk === 'area de lote' || nk.includes('area lote')) k = 'Área lote';
      else if (nk === 'closet' || nk === 'closets') k = 'Closet';
      else if (nk === 'inventario') k = 'Inventario';
      else if (nk === 'celulares' || nk === 'celular propietario' || nk.includes('celular')) k = 'Celulares';
      else if (nk === 'nombre del propietario' || nk === 'propietario') k = 'Nombre del Propietario';
      else if (nk === 'cuanto renta' || nk === 'cuanto renta ($)' || nk === 'renta') k = 'Cuánto Renta ($)';
      else if (nk === 'ascensor') k = 'Ascensor';
      else if (nk === 'numero de cortinas' || nk === 'cortinas') k = 'Número de Cortinas';
      else if (nk === 'aire acondicionado' || nk === 'aire') k = 'Aire Acondicionado';
      else if (nk === 'reja antejardin' || nk === 'reja') k = 'Reja Antejardín';
      else if (nk === 'antiguedad del inmueble' || nk === 'antiguedad' || nk === 'antiguedad inmueble') k = 'Antigüedad del Inmueble';
      ...
      if (!k) {
        ...
        else if (nk.includes('area lote') || nk.includes('area de lote')) k = 'Área lote';
        else if (nk.includes('closet')) k = 'Closet';
        else if (nk.includes('inventario')) k = 'Inventario';
        else if (nk.includes('celulares')) k = 'Celulares';
        else if (nk.includes('propietario')) k = 'Nombre del Propietario';
        else if (nk.includes('renta')) k = 'Cuánto Renta ($)';
        else if (nk.includes('ascensor')) k = 'Ascensor';
        else if (nk.includes('cortina')) k = 'Número de Cortinas';
        else if (nk.includes('aire') && nk.includes('acondicionado')) k = 'Aire Acondicionado';
        else if (nk.includes('reja') || nk.includes('antejardin')) k = 'Reja Antejardín';
        else if (nk.includes('antiguedad')) k = 'Antigüedad del Inmueble';
...
let currentAdminSubTab = 'resumen';
let currentAdminYear = '2026';
let currentAdminMonth = 'MAYO';
...
async function loadAdminData() {
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 7000); // 7s
    
    const url = CRM_SCRIPT_URL + '?action=getAdminData&t=' + Date.now();
  ...
  // Fallback 2: admin_data.json local
  try {
    ...
    console.error('Error fatal al cargar admin_data.json:', e);
  }
}
...
function renderAdministracion() {
  ...
  // Asegurar que el modal de detalle existe en el DOM
  if (!document.getElementById('matrixDetailModal')) {
    const overlay = document.createElement('div');
    overlay.id = 'modalOverlay';
    overlay.className = 'modal-overlay';
    overlay.onclick = closeMatrixCellDetails;
    document.body.appendChild(overlay);

    const modal = document.createElement('div');
    modal.id = 'matrixDetailModal';
  ...
  c.innerHTML = `
    <div class="section-header" style="margin-bottom:15px;">
      ...
      <div style="font-size:12px; color:var(--muted); font-weight:500;">
        Última actualización: ${adminData.last_update ? new Date(adminData.last_update).toLocaleString() : 'N/A'}
      </div>
    </div>
    ...
    <div class="admin-nav-bar">
      <div class="admin-subtabs">
        <button class="admin-subtab-btn ${currentAdminSubTab === 'resumen' ? 'active' : ''}" onclick="changeAdminSubTab('resumen')">📋 Resumen Cobros</button>
        <button class="admin-subtab-btn ${currentAdminSubTab === 'matriz' ? 'active' : ''}" onclick="changeAdminSubTab('matriz')">🗓 Matriz de Pagos</button>
        <button class="admin-subtab-btn ${currentAdminSubTab === 'silvia' ? 'active' : ''}" onclick="changeAdminSubTab('silvia')">🏢 Edificio Silvia</button>
      </div>
      <div class="admin-actions">
        <div class="wa-status-badge">
          <div id="waStatusDot" class="wa-status-dot inactive"></div>
          <span id="waStatusText">Puente WA: Buscando...</span>
        </div>
        <button class="btn btn-primary btn-sm" onclick="syncAllToCalendar()">🗓 Sincronizar Todo (ICS)</button>
        <button class="btn btn-gold btn-sm" onclick="sendBulkReminders()">💬 Enviar Masivos WA</button>
      </div>
    </div>

    <!-- KPI GRID DINÁMICO -->
    <div class="admin-kpi-grid" id="adminKpisContainer">
      <!-- Se llena dinámicamente -->
    </div>

    <!-- BARRA DE ACCIÓN Y FILTROS -->
    <div class="action-bar" style="display: flex; gap: 15px; margin-bottom: 20px; align-items: center; background: rgba(255,255,255,0.01); padding: 12px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.03);">
      <div class="search-box" style="flex: 1; max-width: 320px; position: relative;">
        <input type="text" id="adminPropertySearch" class="form-input" style="padding-left: 35px; height: 38px; border-radius: 10px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); font-size: 13px;" placeholder="Buscar inmueble, dueño o inquilino..." oninput="renderAdministracionContent()">
        <span style="position: absolute; left: 12px; top: 10px; color: var(--muted); font-size: 13px;">🔍</span>
      </div>
      
      <div style="display: flex; gap: 10px; align-items: center; margin-left: auto;">
        <span style="font-size: 11px; color: var(--muted); font-weight: 700; letter-spacing:0.05em;">AÑO:</span>
        <select id="adminYearSelect" class="form-input" style="width: 100px; height: 38px; border-radius: 10px; background: #121212; border: 1px solid rgba(255,255,255,0.08); font-size: 13px; color: #fff; font-weight: 600; cursor:pointer;" onchange="changeAdminYear(this.value)">
          <option value="2023" ${currentAdminYear === '2023' ? 'selected' : ''}>2023</option>
          <option value="2024" ${currentAdminYear === '2024' ? 'selected' : ''}>2024</option>
          <option value="2025" ${currentAdminYear === '2025' ? 'selected' : ''}>2025</option>
          <option value="2026" ${currentAdminYear === '2026' ? 'selected' : ''}>2026</option>
          <option value="2027" ${currentAdminYear === '2027' ? 'selected' : ''}>2027</option>
        </select>
      </div>

      <div id="adminMonthSelectContainer" style="display: ${currentAdminSubTab === 'silvia' ? 'none' : 'flex'}; gap: 10px; align-items: center;">
        <span style="font-size: 11px; color: var(--muted); font-weight: 700; letter-spacing:0.05em; margin-left: 10px;">MES:</span>
        <select id="adminMonthSelect" class="form-input" style="width: 130px; height: 38px; border-radius: 10px; background: #121212; border: 1px solid rgba(255,255,255,0.08); font-size: 13px; color: #fff; font-weight: 600; cursor:pointer;" onchange="changeAdminMonth(this.value)">
          ${["ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "JUNIO", "JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"].map(m => `
            <option value="${m}" ${currentAdminMonth === m ? 'selected' : ''}>${m}</option>
          `).join('')}
        </select>
      </div>

      <button class="btn btn-secondary btn-sm" style="height: 38px; border-radius: 10px; margin-left: 10px;" onclick="refreshAdminData()">⟳ Recargar</button>
    </div>
...
function renderAdministracionContent() {
  ...
  if (currentAdminSubTab === 'silvia') {
    const silviaTx = adminData.silvia_ledger[currentAdminYear] || [];
    let totalRecaudo = 0;
    let totalAbono = 0;
    silviaTx.forEach(t => {
      totalRecaudo += parseFloat(t.recaudo) || 0;
      totalAbono += parseFloat(t.abono) || 0;
    });
    const balanceNeto = totalRecaudo - totalAbono;

    kpisContainer.innerHTML = `
      <div class="admin-kpi-card">
        <div class="admin-kpi-val">${safeFormatP(totalRecaudo)}</div>
        <div class="admin-kpi-lbl">Recaudo Estimado (${currentAdminYear})</div>
        <div class="admin-kpi-sub">Total de administración asignada</div>
      </div>
      <div class="admin-kpi-card" style="border-color: rgba(34,197,94,0.35);">
        <div class="admin-kpi-val" style="color: #22c55e;">${safeFormatP(totalAbono)}</div>
        <div class="admin-kpi-lbl">Total Recaudado / Pagado</div>
        <div class="admin-kpi-sub">🟢 Depósitos confirmados en banco</div>
      </div>
      <div class="admin-kpi-card" style="border-color: ${balanceNeto > 0 ? 'rgba(239,68,68,0.35)' : 'rgba(212,168,75,0.15)'};">
        <div class="admin-kpi-val" style="color: ${balanceNeto > 0 ? '#ef4444' : '#fff'};">${safeFormatP(balanceNeto)}</div>
        <div class="admin-kpi-lbl">Saldo Pendiente</div>
        <div class="admin-kpi-sub">${balanceNeto > 0 ? '🔴 Cartera morosa activa' : '✓ Cuentas completamente al día'}</div>
      </div>
    `;
  } else {
    let totalEsperado = 0;
    ...
    kpisContainer.innerHTML = `
      ...
      <div class="admin-kpi-card" style="border-color: rgba(34,197,94,0.3);">
      ...
      <div class="admin-kpi-card" style="border-color: ${totalPendiente > 0 ? 'rgba(239,68,68,0.3)' : 'rgba(212,168,75,0.15)'};">
      ...
      <div class="admin-kpi-card">
        <div class="admin-kpi-val">${ocupacionPorcentaje}%</div>
        <div class="admin-kpi-lbl">Eficiencia / Ocupación</div>
        <div class="admin-kpi-sub">🟢 ${ocupados} ocupados de ${totalActivos} unidades</div>
      </div>
    `;
  }

  if (currentAdminSubTab === 'resumen') {
    tabContent.innerHTML = renderResumenCobros(filteredProperties);
  } else if (currentAdminSubTab === 'matriz') {
    tabContent.innerHTML = renderMatrizPagos(filteredProperties);
  } else if (currentAdminSubTab === 'silvia') {
    tabContent.innerHTML = renderEdificioSilvia(searchVal);
  }
}
...
function renderResumenCobros(propertiesList) {
  ...
  return `
    <div class="panel-section" style="padding:0; overflow:hidden; border: 1px solid var(--border); border-radius: 16px;">
          ...
          ${propertiesList.map(p => {
            ...
            return `
              <tr>
                ...
                <td>
                  <div style="font-weight:600; color:rgba(255,255,255,0.9);">${p.owner}</div>
                ...
                <td style="text-align:center;">
                  <div style="display:flex; gap:8px; justify-content:center;">
                    <button class="btn btn-sm btn-secondary" style="padding: 6px 10px; font-size: 14px;" 
                            onclick="sendAdminReminder('${p.owner}', '${p.name}', '${p.monthly_rent}', '${currentAdminMonth}', '${currentAdminYear}', ${dueDay})" 
                            title="Enviar recordatorio WhatsApp">
                      💬
                    </button>
                    <button class="btn btn-sm btn-secondary" style="padding: 6px 10px; font-size: 14px;" 
                            onclick="addIndividualToCalendar('${p.name}', '${p.owner}', ${p.monthly_rent}, '${currentAdminYear}', '${currentAdminMonth}', ${dueDay}, ${maxDueDay})" 
...
function renderMatrizPagos(propertiesList) {
  ...
  return `
      ...
      <table class="matrix-table">
        ...
        <tbody>
          ${propertiesList.map(p => {
            ...
            return `
              <tr>
                <td class="cell-prop" title="${p.name} (${p.owner})">
                ...
                ${monthsFull.map(mFull => {
                  const pay = paymentsYear.find(m => m.month === mFull) || { status: 'UNSTARTED', value: 0 };
                  let cellContent = '·';
                  ...
                  return `
                    <td>
                      <div class="matrix-cell status-cell-${pay.status}" 
                           data-tooltip="${tooltipText}" 
                           onclick="openMatrixCellDetails('${p.id}', '${mFull}', '${currentAdminYear}')">
                        ${cellContent}
                      </div>
                    </td>
                  `;
    ...
    <!-- LEYENDA DEL TABLERO -->
    <div style="display:flex; flex-wrap:wrap; gap:15px; margin-top:20px; justify-content:center; background:rgba(255,255,255,0.02); padding:12px; border-radius:10px; border:1px solid rgba(255,255,255,0.04);">
      <div style="display:flex; align-items:center; gap:6px; font-size:11px; font-weight:600; color:rgba(255,255,255,0.8);"><div style="width:12px; height:12px; border-radius:3px; background:rgba(34,197,94,0.12); border:1px solid rgba(34,197,94,0.3);"></div> ✓ Pagado</div>
      <div style="display:flex; align-items:center; gap:6px; font-size:11px; font-weight:600; color:rgba(255,255,255,0.8);"><div style="width:12px; height:12px; border-radius:3px; background:rgba(239,68,68,0.12); border:1px solid rgba(239,68,68,0.35);"></div> $ Pendiente</div>
      <div style="display:flex; align-items:center; gap:6px; font-size:11px; font-weight:600; color:rgba(255,255,255,0.8);"><div style="width:12px; height:12px; border-radius:3px; background:rgba(168,85,247,0.12); border:1px solid rgba(168,85,247,0.3);"></div> ⚠ Preaviso</div>
      <div style="display:flex; align-items:center; gap:6px; font-size:11px; font-weight:600; color:rgba(255,255,255,0.8);"><div style="width:12px; height:12px; border-radius:3px; background:rgba(59,130,246,0.12); border:1px solid rgba(59,130,246,0.3);"></div> ★ Nuevo Contrato</div>
      <div style="display:flex; align-items:center; gap:6px; font-size:11px; font-weight:600; color:rgba(255,255,255,0.8);"><div style="width:12px; height:12px; border-radius:3px; background:rgba(156,163,175,0.08); border:1px solid rgba(156,163,175,0.15);"></div> - Desocupado</div>
      <div style="display:flex; align-items:center; gap:6px; font-size:11px; font-weight:600; color:rgba(255,255,255,0.8);"><div style="width:12px; height:12px; border-radius:3px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.04);"></div> . No Iniciado/Futuro</div>
    </div>
  `;
}
...
function sendAdminReminder(owner, prop, amount, month, year, dueDay) {
  const lead = leads.find(l => l.nombre.toLowerCase().includes(owner.toLowerCase()) || owner.toLowerCase().includes(l.nombre.toLowerCase()));
  const phone = lead ? lead.celular : prompt(`No se encontró el teléfono del arrendatario para ${owner}. Por favor ingrésalo (código de país incluido sin '+', ej: 573001234567):`);
  
  if (!phone) return;
  
  const cleanPhone = phone.replace(/[^\d]/g, '');
  const msg = `Hola ${owner}, te escribimos de ICDE Inmobiliaria para recordarte el pago del canon de arrendamiento de la propiedad *${prop}* por valor de *${safeFormatP(amount)}* correspondiente a *${month}/${year}*. Recuerda que el día límite de pago es el ${dueDay}. ¡Muchas gracias!`;
  ...
  if (waBridgeConnected) {
    ...
    fetch('http://localhost:3000/send-reminder', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ number: cleanPhone, name: owner, property: prop, amount: amount, dueDate: dueDay })
    }).then(res => {
      if (res.ok) {
...
async function sendBulkReminders() {
  ...
  if (waBridgeConnected) {
    ...
    for (const item of pending) {
      const p = item.prop;
      const lead = leads.find(l => l.nombre.toLowerCase().includes(p.owner.toLowerCase()) || p.owner.toLowerCase().includes(l.nombre.toLowerCase()));
      const phone = lead ? lead.celular : null;
      if (!phone) continue;
  ...
  properties.forEach(p => {
    ...
    yearPayments.forEach((pay, index) => {
      if (pay.status === 'PENDING' || pay.status === 'PAID' || pay.status === 'PREAVISO' || pay.status === 'NEW_CONTRACT') {
        ...
        icsContent += `SUMMARY:Cobro Arriendo - ${p.name} (${p.owner})\n`;
        icsContent += `DESCRIPTION:Cobro de canon de arrendamiento de la propiedad ${p.name} por valor de ${safeFormatP(rent)}. Inquilino/Propietario: ${p.owner}. Pagar antes del dia ${maxDueDay}.\n`;
...
function addIndividualToCalendar(propName, owner, rent, year, month, dueDay, maxDueDay) {
  const monthsMap = {
  ...
  const title = `Cobro Arriendo - ${propName} (${owner})`;
  const details = `Cobro de canon de arrendamiento de la propiedad ${propName} por valor de ${safeFormatP(rent)}. Inquilino: ${owner}. Plazo maximo de pago: dia ${maxDueDay}.`;
...
function openMatrixCellDetails(propId, monthName, year) {
  ...
  modal.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:18px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:12px;">
      <div style="font-size:16px; font-weight:800; color:var(--gold); display:flex; align-items:center; gap:8px;">
        🏢 Detalle de Cobro
      </div>
      <button onclick="closeMatrixCellDetails()" style="background:transparent; border:none; color:var(--muted); font-size:18px; cursor:pointer; font-weight:700; transition:color 0.2s;" onhover="this.style.color='#fff'">×</button>
    </div>
    
    <div style="display:flex; flex-direction:column; gap:14px; font-size:13px; color:rgba(255,255,255,0.9); max-height: 480px; overflow-y: auto; padding-right: 4px;">
      <div>
        <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Inmueble</div>
        <div style="font-size:15px; font-weight:700; color:#fff;">${p.name}</div>
      </div>
      
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
        <div>
          <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Mes / Año</div>
          <div style="font-weight:600; color:#fff;">${monthName} ${year}</div>
        </div>
        <div>
          <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Estado</div>
          <span class="${getStatusBadgeClass(pay.status)}" style="padding:3px 8px; font-size:10px;">${getStatusLabel(pay.status)}</span>
        </div>
      </div>

      <div>
        <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Inquilino / Dueño</div>
        <div style="font-weight:600; color:#fff;">${p.owner}</div>
      </div>

      <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
        <div>
          <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Valor Canon</div>
          <div style="font-weight:700; color:var(--gold); font-size:14px;">${safeFormatP(p.monthly_rent)}</div>
        </div>
        <div>
          <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Monto Transado</div>
          <div style="font-weight:700; color:#fff;">${pay.value && parseFloat(pay.value) > 0 ? safeFormatP(pay.value) : '—'}</div>
        </div>
      </div>

      <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
        <div>
          <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Rango de Pago</div>
          <div style="font-weight:600; color:#fff;">Día ${dueDay} al ${maxDueDay}</div>
        </div>
        <div>
          <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Inicio Contrato</div>
          <div style="font-weight:600; color:#fff;">${p.start_date || 'N/A'}</div>
        </div>
      </div>

      ${p.deposit > 0 ? `
        <div>
          <div style="font-size:10.5px; color:var(--muted); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Depósito en Garantía</div>
          <div style="font-weight:600; color:#22c55e;">${safeFormatP(p.deposit)}</div>
        </div>
      ` : ''}

      ${p.damage_notes ? `
        <div style="background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.06); padding:8px; border-radius:8px;">
          <div style="font-size:10px; color:#a855f7; text-transform:uppercase; font-weight:700; margin-bottom:3px;">Daños / Depósito</div>
          <div style="font-size:11px; color:#fff; font-style:italic;">${p.damage_notes}</div>
        </div>
      ` : ''}

      ${p.increase_notes ? `
        <div style="background:rgba(212,168,75,0.03); border:1px solid rgba(212,168,75,0.08); padding:8px; border-radius:8px;">
          <div style="font-size:10px; color:var(--gold); text-transform:uppercase; font-weight:700; margin-bottom:3px;">Incrementos y Notas</div>
          <div style="font-size:11px; color:rgba(255,255,255,0.85);">${p.increase_notes.replace(/\|/g, '<br>')}</div>
        </div>
      ` : ''}

      <!-- Sección de Edición de Pago Bilateral -->
      <div style="margin-top:8px; border-top:1px solid rgba(255,255,255,0.1); padding-top:12px;">
        <div style="font-size:11px; color:var(--gold); text-transform:uppercase; font-weight:700; margin-bottom:8px; display:flex; align-items:center; gap:5px;">
          ✏️ Registrar Pago (Sincronización Nube)
        </div>
        <div style="display:grid; grid-template-columns:1.2fr 1fr; gap:10px; margin-bottom:10px;">
          <div>
            <label style="font-size:10px; color:var(--muted); display:block; margin-bottom:3px;">Nuevo Estado</label>
            <select id="editPayStatus" style="width:100%; background:#222; border:1px solid rgba(255,255,255,0.15); border-radius:6px; color:#fff; padding:6px; font-size:12px; outline:none; height:32px;">
              ...
              <option value="VACANT" ${pay.status === 'VACANT' ? 'selected' : ''}>Desocupado</option>
              <option value="PREAVISO" ${pay.status === 'PREAVISO' ? 'selected' : ''}>Preaviso</option>
              <option value="NEW_CONTRACT" ${pay.status === 'NEW_CONTRACT' ? 'selected' : ''}>Nuevo Contrato</option>
              <option value="FUTURE" ${pay.status === 'FUTURE' ? 'selected' : ''}>Futuro (Vacío)</option>
            </select>
          </div>
          <div>
            <label style="font-size:10px; color:var(--muted); display:block; margin-bottom:3px;">Valor Recibido</label>
            <input type="number" id="editPayValue" value="${pay.value && parseFloat(pay.value) > 0 ? parseFloat(pay.value) : ''}" placeholder="${p.monthly_rent}" style="width:100%; background:#222; border:1px solid rgba(255,255,255,0.15); border-radius:6px; color:#fff; padding:6px 8px; font-size:12px; outline:none; height:32px; box-sizing:border-box;">
          </div>
        </div>
        <button class="btn btn-gold btn-sm" id="btnSaveAdminPay" style="width:100%; justify-content:center; gap:6px; font-weight:700; height:34px;" onclick="saveAdminPaymentStatus('${p.id}', '${p.name.replace(/'/g, "\\'")}', '${monthName}', '${year}')">
          💾 Guardar en la Nube
        </button>
      </div>
    </div>

    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:14px; border-top:1px solid rgba(255,255,255,0.08); padding-top:12px;">
      <button class="btn btn-secondary btn-sm" style="display:flex; align-items:center; justify-content:center; gap:6px; font-weight:700; background:rgba(255,255,255,0.05); color:#fff; border:1px solid rgba(255,255,255,0.1);" 
              onclick="sendAdminReminder('${p.owner}', '${p.name.replace(/'/g, "\\'")}', '${p.monthly_rent}', '${monthName}', '${year}', ${dueDay})">
        💬 WhatsApp
      </button>
      <button class="btn btn-primary btn-sm" style="display:flex; align-items:center; justify-content:center; gap:6px; font-weight:700;" 
              onclick="addIndividualToCalendar('${p.name.replace(/'/g, "\\'")}', '${p.owner}', ${p.monthly_rent}, '${year}', '${monthName}', ${dueDay}, ${maxDueDay})">
        📅 Calendar
      </button>
    </div>
  `;
...
async function saveAdminPaymentStatus(propId, propName, monthName, year) {
  ...
  // Mapear el valor según el estado
  let finalValue = '';
  if (status === 'PAID') {
  ...
  } else if (status === 'VACANT' || status === 'FUTURE') {
    finalValue = 'DESOCUPADO';
  } else if (status === 'PREAVISO') {
    finalValue = 'PREAVISO';
  } else if (status === 'NEW_CONTRACT') {
    finalValue = 'CONTRATO NUEVO';
  } else if (status === 'NO_RENEW') {
    finalValue = 'NO RENOVARA';
  } else {
    finalValue = rawValue || '-';
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (resData.success) {
        ...
        // Actualizar localmente en memoria para renderizado inmediato
        const p = adminData.properties.find(x => x.id === propId);
        if (p) {
          ...
          if (mPay) {
            mPay.status = status;
            mPay.value = finalValue;
          } else {
            p.payments[year].push({
              month: monthName,
              value: finalValue,
              status: status
            });
        ...
        closeMatrixCellDetails();
        renderAdministracion();
      } else {
        throw new Error(resData.error || 'Error desconocido del servidor');
      }
    } else {
      throw new Error('Error de conexión HTTP');
    }
  } catch (err) {
    ...
    // Fallback local: Guardar al menos localmente
    const p = adminData.properties.find(x => x.id === propId);
    if (p) {
      if (!p.payments[year]) p.payments[year] = [];
      let mPay = p.payments[year].find(m => m.month === monthName);
...
function closeMatrixCellDetails() {
  const modal = document.getElementById('matrixDetailModal');
  const overlay = document.getElementById('modalOverlay');
  if (modal && overlay) {
...
function initMapa(pfx) {
  ...
  // Map click reverse geocoding for Street View bottom card
  ...
  bottomCard.innerHTML = `
    ...
    <div class="bottom-card-thumbnail" id="bottomCardThumbnail">
      <img id="bottomCardImg" src="" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.style.display='none'"/>
      <div class="bottom-card-thumbnail-overlay">
         <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#ffffff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom: 2px; filter: drop-shadow(0 1px 2px rgba(0,0,0,0.5));"><path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38l5.67-5.67"/></svg>
         <span>Street View</span>
         <span class="bottom-card-google-text">Google</span>
      </div>
    </div>
    ...
    <div class="bottom-card-actions">
      <button class="bottom-card-action-btn primary" id="bottomCardDirBtn" title="Cómo llegar">
         <svg viewBox="0 0 24 24"><path d="M21.71 11.29l-9-9a.996.996 0 0 0-1.41 0l-9 9a.996.996 0 0 0 0 1.41l9 9c.39.39.9.39 1.29 0l9-9a.996.996 0 0 0 .12-1.41z" fill="#ffffff"/><path d="M14 14.5V12h-4v3H8v-5h6V7.5l4 4-4 3z" fill="#00838f"/></svg>
      </button>
      <button class="bottom-card-action-btn share" id="bottomCardShareBtn" title="Compartir ubicación">
         <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
      </button>
    </div>
  `;
        ...
        autocompleteService.getPlacePredictions(request, (predictions, status) => {
          ...
          if (status === google.maps.places.PlacesServiceStatus.OK && predictions) {
            predictions.slice(0, 5).forEach(place => {
              // Split into main text and secondary text (context), identical to Google Maps UI
  ...
  function renderSuggestions(list) {
    ...
    list.forEach(item => {
      ...
      row.addEventListener('click', (e) => {
        ...
        if (item.placeId && typeof google !== 'undefined') {
          const geocoder = new google.maps.Geocoder();
...
function compartirPropiedad(codigo, btn){
  ...
  if(!p) return;
  const url = window.location.origin + '/propiedad/' + generarSlugPropiedad(p) + '.html';
...
function abrirModalProp(codigoOrProp){
  ...
  const items = [
    ...
    ['Celulares Propietario', p['Celulares']],
    ['Cuánto Renta', p['Cuánto Renta ($)'] ? `$${Number(p['Cuánto Renta ($)']).toLocaleString('es-CO')}` : ''],
    ['Inventario', p['Inventario']]
  ];
  specsTable.innerHTML = items.map(([k,v]) => `<tr><td>${k}</td><td>${v||'—'}</td></tr>`).join('');
...
function initMpMapa(p) {
  ...
  // Update Street View thumbnail in the modal map corner
...
function sincronizarAliadosConPropiedades() {
  ...
  const nombresExistentes = new Set(aliados.map(a => norm(a.nombre)));
    ...
    <div style="background: var(--bg2); border: 1px solid var(--border); border-radius: 16px; padding: 24px;">
      ...
      <div class="form-grid" style="grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 15px; margin-bottom: 24px;">
        <div class="form-group" style="position:relative;"><label class="form-label">Código</label>
          ...
          <div id="gp_codigo_dropdown" style="display:none; position:absolute; left:0; right:0; top:calc(100% + 2px); background:#141210; border:1px solid rgba(212,168,75,0.3); border-radius:10px; padding:10px; z-index:100; box-shadow:0 10px 30px rgba(0,0,0,0.8); cursor:pointer;" onclick="autofillPropiedadPorCodigo()">
          </div>
        </div>
        ...
        <div class="form-group"><label class="form-label">Zona</label><input class="form-input" id="gp_zona" type="text" placeholder="Ej: Sur"/></div>
        ...
        <div class="form-group"><label class="form-label">Ubicación / Dirección</label><input class="form-input" id="gp_ubicacion" type="text" placeholder="Dirección exacta"/></div>
        ...
        <div class="form-group"><label class="form-label">Pisos</label><input class="form-input" id="gp_pisos" type="number" placeholder="Nº de pisos" value="1"/></div>
        ...
        <div class="form-group"><label class="form-label">Área (m²)</label><input class="form-input" id="gp_area" type="number" placeholder="m²"/></div>
        ...
        <div class="form-group"><label class="form-label">Comuna</label><input class="form-input" id="gp_comuna" type="text" placeholder="Comuna"/></div>
        ...
        <div class="form-group"><label class="form-label">Celulares (Propietario)</label><input class="form-input" id="gp_celulares" type="text" placeholder="Celulares"/></div>
        ...
        <div class="form-group"><label class="form-label">Número de Cortinas</label><input class="form-input" id="gp_cortinas" type="number" placeholder="Nº de cortinas" value="0"/></div>
        <div class="form-group"><label class="form-label">Aire Acondicionado</label><input class="form-input" id="gp_aire_acondicionado" type="text" placeholder="Ej: SI / NO / Cantidad"/></div>
...
async function validarCodigoExistente(codigo) {
  ...
  // Mostrar estado de carga si no tenemos la matriz
  if (!icdeMatrixText && !loadingMatrix) {
    loadingMatrix = true;
    if (msg) {
      msg.style.display = 'inline-block';
      msg.style.color = '#d4a84b';
      msg.textContent = '⌛ Consultando matriz...';
    }
    try {
      const url = "https://docs.google.com/spreadsheets/d/1o4W4k1rxPq7t20RTd-1tC10ObsRUG3k-WewOKHshJdg/export?format=csv&gid=1129470117";
      const resp = await fetch(url);
      icdeMatrixText = await resp.text();
    } catch (e) {
      console.error('Error al consultar la matriz:', e);
    } finally {
      loadingMatrix = false;
    }
  }
  ...
  // 1. Siempre revisar en allProps primero para tener datos base
  ...
  if (propFromAllProps) {
    // Clonamos para no mutar el original de allProps directamente
    matchedProp = {...propFromAllProps};
  }

  // 2. SIEMPRE buscar en la matriz CSV para obtener el link de Google Fotos real y fresco
  if (icdeMatrixText) {
    const lines = icdeMatrixText.split(/\r?\n/);
    if (lines.length > 0) {
      // Autodetectar si el separador es coma (,) o punto y coma (;)
      let sep = ',';
      const commas = (lines[0].match(/,/g) || []).length;
      const semicolons = (lines[0].match(/;/g) || []).length;
      if (semicolons > commas) sep = ';';
      
      const headers = lines[0].split(sep).map(h => h.trim().replace(/^"|"$/g, ''));
      const codeIdx = headers.findIndex(h => h.toLowerCase() === 'código' || h.toLowerCase() === 'codigo');
      const gfIdx = headers.findIndex(h => h.trim().toLowerCase() === 'google fotos');
      
      if (codeIdx !== -1) {
        // Regex para separar por el delimitador detectado ignorando lo que esté entre comillas
        const cellRegex = new RegExp(`${sep}(?=(?:(?:[^"]*"){2})*[^"]*$)`);
        
        for (let i = 1; i < lines.length; i++) {
          const line = lines[i];
          if (!line.trim()) continue;
          const cells = line.split(cellRegex);
          const valStr = cells[codeIdx] ? cells[codeIdx].trim().replace(/^"|"$/g, '') : '';
          
          if (valStr.toLowerCase() === codigo.toLowerCase() || (valStr && !isNaN(valStr) && !isNaN(codigo) && parseFloat(valStr) === parseFloat(codigo))) {
            const liveGoogleFotos = gfIdx !== -1 && cells[gfIdx] ? cells[gfIdx].trim().replace(/^"|"$/g, '') : '';
            
            if (matchedProp) {
              // Forzar a que use el de la matriz en vivo
              matchedProp['Google Fotos'] = liveGoogleFotos;
            } else {
              // Si no existía en allProps, construimos el objeto completo desde la matriz
              matchedProp = {};
              headers.forEach((h, idx) => {
                matchedProp[h] = cells[idx] ? cells[idx].trim().replace(/^"|"$/g, '') : '';
              });
              matchedProp['Google Fotos'] = liveGoogleFotos;
            }
            break;
  ...
  if (matchedProp) {
    window.currentMatchedProp = matchedProp;
    if (msg) {
      msg.style.display = 'inline-block';
    ...
    if (dropdown) {
      ...
      const name = matchedProp['Nombre'] || matchedProp['nombre'] || 'Sin nombre';
      const barrio = matchedProp['Barrio'] || matchedProp['barrio'] || '';
      const zona = matchedProp['Zona'] || matchedProp['zona'] || '';
      const code = matchedProp['Código'] || matchedProp['codigo'] || codigo;
      ...
      dropdown.innerHTML = `
        ...
        <div style="font-size:10px; color:var(--gold); margin-top:6px; border-top:1px solid rgba(255,255,255,0.05); padding-top:4px;">🖱️ Click para autocompletar y editar</div>
      `;
    }
  } else {
    window.currentMatchedProp = null;
...
function autofillPropiedadPorCodigo() {
  const p = window.currentMatchedProp;
  if (!p) return;
  ...
  setVal('gp_contrato', p['Contrato'] || p['contrato'] || 'Venta');
  ...
  setVal('gp_conjunto', p['Conjunto / Edificio'] || p['conjunto'] || p['Edificio']);
  setVal('gp_zona', p['Zona'] || p['zona']);
  setVal('gp_ciudad', p['Ciudad'] || p['ciudad'] || 'Cali');
  setVal('gp_estrato', p['Estrato'] || p['estrato']);
  setVal('gp_ubicacion', p['Ubicación / Dirección'] || p['ubicacion'] || p['Dirección']);
  ...
  setVal('gp_area', p['Área (m²)'] || p['area'] || p['Área construida']);
  
  // Nuevos campos para coincidir con la matriz
  setVal('gp_cocina', p['Cocina'] || 'No');
  setVal('gp_piscina', p['Piscina'] || 'No tiene');
  setVal('gp_pisos', p['Pisos'] || '1');
  setVal('gp_ubicacion_posicion', p['Ubicación'] || 'Medianera');
  
  // SOLUCIÓN ESTRICTA PERO INTELIGENTE: Solo leer de la columna exacta 'Google Fotos',
  // pero si por algún motivo viene vacío (ej. fallo de fetch y fallback a allProps), 
  // buscar en las otras columnas si alguna contiene un enlace real de álbum de Google Fotos.
  ...
  setVal('gp_inmobiliaria', p['Inmobiliaria / Aliado'] || p['inmobiliaria'] || p['Inmob'] || '');
  setVal('gp_administracion', p['Administración ($)'] || p['administracion'] || p['Administración']);
  setVal('gp_rentabilidad', p['Retorno inversión (Rentabilidad)'] || p['rentabilidad'] || p['Retorno de la inversión']);
  
  setVal('gp_comuna', p['Comuna'] || '');
  setVal('gp_area_lote', p['Área lote'] || p['Área de lote'] || p['areaLote'] || '');
  setVal('gp_closets', p['Closet'] || p['Closets'] || p['closet'] || '0');
  setVal('gp_inventario', p['Inventario'] || 'NO');
  setVal('gp_celulares', p['Celulares'] || '');
  setVal('gp_propietario', p['Nombre del Propietario'] || p['propietario'] || '');
  
  const rentaEl = document.getElementById('gp_renta');
  if (rentaEl) {
    rentaEl.value = p['Cuánto Renta ($)'] || p['renta'] || p['Cuánto renta'] || '';
    formatPrecioOnInput(rentaEl);
  }
  
  setVal('gp_ascensor', p['Ascensor'] || 'NO');
  setVal('gp_cortinas', p['Número de Cortinas'] || p['cortinas'] || '0');
  setVal('gp_aire_acondicionado', p['Aire Acondicionado'] || p['aire_acondicionado'] || '');
  setVal('gp_reja_antejardin', p['Reja Antejardín'] || p['rejaAntejardin'] || 'NO');
  setVal('gp_antiguedad', p['Antigüedad del Inmueble'] || p['Antigüedad'] || p['antiguedad'] || '');
  setVal('gp_patio', p['Patio'] || 'NO');
  setVal('gp_dimensiones', p['Dimensiones'] || '');
...
function resetNuevaPropForm() {
  ...
  ids.forEach(id => {
    ...
    if (el) {
      ...
      else if (id === 'gp_contrato') el.value = 'Venta';
...
async function gpAutoGenerarIA() {
  if (!settings.geminiKey) {
    toast('Por favor configura tu Gemini API Key en Configuración ⚙️', 'error');
    return;
  }
...
function resetNuevaPropForm() {
  const ids = [
    ...
    'gp_comuna', 'gp_area_lote', 'gp_celulares', 'gp_propietario', 'gp_renta', 'gp_cortinas', 'gp_aire_acondicionado', 'gp_antiguedad', 'gp_dimensiones'
  ];
  ids.forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  
  const selectIds = [
    'gp_tipo', 'gp_contrato', 'gp_estrato', 'gp_habitaciones', 'gp_banos', 'gp_garaje', 'gp_piscina', 'gp_cocina',
    'gp_closets', 'gp_inventario', 'gp_ascensor', 'gp_reja_antejardin', 'gp_patio'
  ];
...
async function ejecutarGuardadoPropiedad(prop) {
  ...
  try {
    const scriptUrl = 'https://script.google.com/macros/s/AKfycbz-HipJ53KIf2JD1q9BUIBFUB45o4wRYcjvlqUbpg9TDAGK0q3hNQcSrV23dMCWaTgXcQ/exec';
...
async function swExtraerLinkIA() {
  ...
  if (!settings.geminiKey) {
    toast('Configura tu Gemini API Key en Configuración ⚙️', 'error');
    return;
  }
  ...
  try {
    ...
    const geminiRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${settings.geminiKey}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }] })
    });
    ...
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${settings.geminiKey}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [
            { text: prompt },
            { inlineData: { mimeType: 'image/png', data: window.pastedScreenshotBase64 } }
          ]
        }]
      })
    });
    ...
    <div class="form-grid" style="grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 15px; margin-bottom: 24px;">
      ...
      <div class="form-group"><label class="form-label">Contrato</label>
        <select class="form-input ${emptyClass(data.contrato)}" id="gp_contrato">
          <option value="Venta" ${data.contrato==='Venta'?'selected':''}>Venta</option>
          <option value="Arriendo" ${data.contrato==='Arriendo'?'selected':''}>Arriendo</option>
          <option value="Venta/Arriendo" ${data.contrato==='Venta/Arriendo'?'selected':''}>Venta/Arriendo</option>
        </select>
      </div>
      ...
      <div class="form-group"><label class="form-label">Zona</label><input class="form-input ${emptyClass(data.zona)}" id="gp_zona" type="text" value="${emptyCheck(data.zona)}"/></div>
      ...
      <div class="form-group"><label class="form-label">Ubicación</label><input class="form-input ${emptyClass(data.ubicacion)}" id="gp_ubicacion" type="text" value="${emptyCheck(data.ubicacion)}"/></div>
      <div class="form-group"><label class="form-label">Habitaciones</label>
      ...
      <div class="form-group"><label class="form-label">Área (m²)</label><input class="form-input ${emptyClass(data.area)}" id="gp_area" type="number" value="${emptyCheck(data.area)}"/></div>
      ...
      <div class="form-group"><label class="form-label">Comuna</label><input class="form-input ${emptyClass(data.comuna)}" id="gp_comuna" type="text" value="${emptyCheck(data.comuna)}" placeholder="Comuna"/></div>
      ...
      <div class="form-group"><label class="form-label">Inventario</label>
        <select class="form-input ${emptyClass(data.inventario)}" id="gp_inventario">
          ...
          <option value="SI" ${normSiNo(data.inventario) === 'SI' ? 'selected' : ''}>SI</option>
        </select>
      </div>
      <div class="form-group"><label class="form-label">Celulares (Propietario)</label><input class="form-input ${emptyClass(data.celulares)}" id="gp_celulares" type="text" value="${emptyCheck(data.celulares)}" placeholder="Celulares"/></div>
      ...
      <div class="form-group"><label class="form-label">Número de Cortinas</label><input class="form-input ${emptyClass(data.cortinas)}" id="gp_cortinas" type="number" value="${data.cortinas !== undefined && data.cortinas !== null ? data.cortinas : 0}" placeholder="Nº de cortinas"/></div>
      <div class="form-group"><label class="form-label">Aire Acondicionado</label><input class="form-input ${emptyClass(data.aireAcondicionado)}" id="gp_aire_acondicionado" type="text" value="${emptyCheck(data.aireAcondicionado)}" placeholder="Ej: SI / NO / Cantidad"/></div>
      <div class="form-group"><label class="form-label">Reja Antejardín</label>
      ...
      <div class="form-group"><label class="form-label">Patio</label>
        <select class="form-input ${emptyClass(data.patio)}" id="gp_patio">
          <option value="NO" ${normSiNo(data.patio) === 'NO' ? 'selected' : ''}>NO</option>
          <option value="SI" ${normSiNo(data.patio) === 'SI' ? 'selected' : ''}>SI</option>
        </select>
      </div>
...
function abrirStreetView(lat, lng) {
  ...
  try {
    ...
    // Resize event trigger to ensure panorama fits perfectly
    google.maps.event.trigger(panorama, 'resize');
  } catch (e) {
    console.error("Error loading Google Street View:", e);
    toast("No se pudo cargar Street View en esta ubicación", "error");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (closeBtn) {
    closeBtn.addEventListener('click', () => {
      ...
      if (panoramaDiv) panoramaDiv.innerHTML = ''; // reset panorama
    });
  }
});
...
function mostrarBottomCard(lat, lng, main, sub, full) {
  ...
  // Set street view preview image (enlarged and scale=2 for HD)
