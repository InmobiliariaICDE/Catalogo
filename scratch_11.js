...
let nlShowData = false; // Por defecto colapsado (se despliega al hacer clic)
  ...
  wrap.innerHTML = `
    <div class="pc-filtros-wrapper">
      <div class="filtros-grid-nueva" id="fnGrid_${pfx}">
        ...
        ${pfx==='nl' ? `<button class="btn-limpiar-pc btn-limpiar-en-grid" onclick="fnLimpiar('${pfx}')">❌ Limpiar filtros</button>` : ''}
        <div class="pc-buscar-input-wrap fn-grid-search">
          <span class="bn-icon">
        ...
        <button class="btn-generar-pc" id="btnGen_${pfx}" onclick="fnGenerarLink('${pfx}', this)">🔗&thinsp;Generar link</button>
        ${pfx==='nl' ? `<button class="btn-datos-en-grid btn-datos-bottom" onclick="toggleNlData()"><span class="btn-datos-lbl">📤 Enviar propuestas</span><span class="btn-datos-chevron">&#9660;</span></button>` : `<button class="btn-limpiar-pc btn-limpiar-desktop" onclick="fnLimpiar('${pfx}')">❌ Limpiar filtros</button>`}
      </div>
...
function fnTieneFiltrosActivos(pfx){
  const st = getFSt(pfx);
  if(!st) return false;

  // 1. Campos multiselect / checkbox
  for(let c of FCAMPOS){
    if(c.key === 'rangoPrecio') continue;
    const v = st[c.key];
    if(Array.isArray(v) && v.length > 0) return true;
  }

  // 2. Rango de precio (activo solo si se modifica respecto al rango por defecto)
  const maxLimit = typeof MAX_CATALOG_PRICE !== 'undefined' ? MAX_CATALOG_PRICE : 5000000000;
  if((st.minPrice !== null && st.minPrice > 0) || (st.maxPrice !== null && st.maxPrice < maxLimit)) {
    return true;
  }

  // 3. Campo de búsqueda de texto
  const busInput = document.getElementById(`fnBus_${pfx}`);
  const busVal = busInput ? busInput.value.trim() : (st.buscar || '').trim();
  if(busVal !== '') return true;

  // 4. Categorías KMZ
  if(Array.isArray(st.kmzCategory) && st.kmzCategory.length > 0) return true;

  return false;
}

function fnActualizarEstadoBotonLimpiar(pfx){
  const tiene = fnTieneFiltrosActivos(pfx);
  const wrap = document.getElementById(`fnGrid_${pfx}`)?.closest('.pc-filtros-wrapper') || document;
  const btns = wrap.querySelectorAll('.btn-limpiar-pc, .btn-limpiar-en-grid, .btn-limpiar-desktop');
  btns.forEach(b => {
    if(tiene) {
      b.classList.add('has-filters');
    } else {
      b.classList.remove('has-filters');
    }
  });
}
...
function fnActualizarConteo(pfx){
  ...
  fnActualizarEstadoBotonLimpiar(pfx);
}
...
/* ═══════════════════════════════════════════════
   LOGIN
═══════════════════════════════════════════════ */
window.doLogin = function(isAuto = false){
  if (!isAuto) {
...
// Auto-ingreso automático si ya ha iniciado sesión en este dispositivo
document.addEventListener('DOMContentLoaded', () => {
  if (localStorage.getItem('icde_logged_in') === 'true') {
    doLogin(true);
  }
  initServiceWorkerNotif();
});

// ── SISTEMA DE NOTIFICACIONES EN SEGUNDO PLANO (SERVICE WORKER & SYSTEM PUSH) ──
let swRegistration = null;

async function initServiceWorkerNotif() {
  if ('serviceWorker' in navigator) {
    try {
      swRegistration = await navigator.serviceWorker.register('/sw.js', { scope: '/' });
      console.log('[Service Worker] Registrado correctamente:', swRegistration.scope);
      
      if (navigator.serviceWorker.controller) {
        navigator.serviceWorker.controller.postMessage({
          type: 'SET_CONFIG',
          crmUrl: typeof CRM_SCRIPT_URL !== 'undefined' ? CRM_SCRIPT_URL : ''
        });
      }

      if ('periodicSync' in swRegistration) {
        try {
          await swRegistration.periodicSync.register('icde-check-updates', {
            minInterval: 15 * 1000
          });
        } catch (e) {}
      }
      ...
      console.warn('[Service Worker] No se pudo registrar sw.js:', err);
    }
  }
}

function actualizarEstadoBotonNotif() {
  const btn = document.getElementById('btnNotifCelular');
  if (!btn) return;
  ...
  if (Notification.permission === 'granted') {
    btn.innerHTML = '🔔 Notificaciones Activas en Celular (Verificado)';
    btn.style.background = 'rgba(34,197,94,0.15)';
    btn.style.borderColor = 'rgba(34,197,94,0.4)';
    btn.style.color = '#22c55e';
    btn.title = 'Notificaciones activas en segundo plano';
  } else if (Notification.permission === 'denied') {
    btn.innerHTML = '🔕 Notificaciones Bloqueadas (Revisa Permisos)';
    btn.style.background = 'rgba(239,68,68,0.15)';
    btn.style.borderColor = 'rgba(239,68,68,0.4)';
    btn.style.color = '#ef4444';
    btn.title = 'Notificaciones bloqueadas (Permite en Ajustes de Navegador)';
  } else {
    btn.innerHTML = '🔔 Activar Notificaciones en Celular';
    btn.style.background = 'rgba(212,168,75,0.15)';
    btn.style.borderColor = 'var(--gold)';
    btn.style.color = 'var(--gold)';
    btn.title = 'Activar notificaciones en segundo plano';
  }
}
...
async function solicitarPermisoNotificaciones() {
  ...
  if (Notification.permission === 'granted') {
    actualizarEstadoBotonNotif();
    toast('🟢 Notificaciones activas en este celular. Enviando prueba...', 'success');
    enviarNotificacionSistema('🔔 Prueba ICDE', 'Las notificaciones están funcionando correctamente en tu celular.');
    return;
  }

  if (Notification.permission === 'denied') {
    actualizarEstadoBotonNotif();
    alert('⚠️ Las notificaciones están bloqueadas en tu navegador móvil.\n\nPara activarlas:\n1. Toca el candado 🔒 o sintonizador a la izquierda de la dirección web (URL) en Chrome.\n2. Entra a "Permisos del sitio" ➔ "Notificaciones".\n3. Cambia la opción a "PERMITIR".\n4. Recarga la página.');
    return;
  }
  ...
  try {
    ...
    actualizarEstadoBotonNotif();
    if (perm === 'granted') {
      toast('¡Notificaciones activadas correctamente! 🟢', 'success');
      enviarNotificacionSistema('🟢 ICDE Notificaciones Activas', 'Recibirás alertas en la pantalla de tu celular.');
    } else if (perm === 'denied') {
      alert('⚠️ Se han bloqueado las notificaciones.\n\nSi deseas recibirlas, por favor permite el acceso desde el icono del candado 🔒 en la barra de direcciones de Chrome.');
    }
  } catch(e) {
...
async function enviarWebPushDirecto(titulo, cuerpo) {
  try {
    const apiKey = (typeof settings !== 'undefined' && settings.oneSignalApiKey) || '';
    if (!apiKey) return;

    const payload = {
      app_id: 'ebb863b8-75a9-4f85-931d-c98e7c9a00ee',
      included_segments: ['All'],
      headings: { es: titulo, en: titulo },
      contents: { es: cuerpo, en: cuerpo },
      url: 'https://inmobiliariaicde.web.app/admin.html',
      chrome_web_icon: 'https://i.imgur.com/s3dvfne.png'
    };
...
async function programarWebPushNube(titulo, cuerpo, targetDate, alertKey) {
  try {
    const sentMem = obtenerAlertasEnviadasMemoria();
    if (sentMem[alertKey]) return;

    const apiKey = (typeof settings !== 'undefined' && settings.oneSignalApiKey) || '';
    if (!apiKey) return;

    const sendAfterStr = targetDate.toISOString();
    const payload = {
      app_id: 'ebb863b8-75a9-4f85-931d-c98e7c9a00ee',
      included_segments: ['All'],
      headings: { es: titulo, en: titulo },
    ...
    fetch('https://onesignal.com/api/v1/notifications', {
      ...
      if (res.ok) guardarAlertaEnviadaMemoria(alertKey);
    }).catch(e => {});
  } catch(e) {}
}

function probarNotificacionBackground10s() {
  if (!('Notification' in window) || Notification.permission !== 'granted') {
    alert('Primero activa las notificaciones usando el botón de arriba.');
    return;
  }

  toast('⏱️ ¡Prueba de 10 seg iniciada! Sal de Chrome o bloquea el celular AHORA.', 'info');
    ...
    navigator.serviceWorker.ready.then(reg => {
      if (reg && reg.active) {
        reg.active.postMessage({
          ...
          title: '🔔 Alerta ICDE Inmobiliaria',
          body: '¡Prueba Exitosa! Notificación recibida con el celular bloqueado.',
          delayMs: 10000
        });
      }
    });
  ...
  setTimeout(() => {
    enviarNotificacionSistema('🔔 Alerta ICDE Inmobiliaria', '¡Prueba Exitosa! Notificación y sonido recibidos en segundo plano.');
  }, 10000);
}
...
function obtenerVozLeonidas(voices) {
  if (!voices || voices.length === 0) return null;
  const esVoices = voices.filter(v => v.lang.toLowerCase().startsWith('es'));
  
  const maleNames = ['jorge', 'raul', 'alvaro', 'david', 'mateo', 'pablo', 'julio', 'humberto', 'yago', 'jose', 'hombre', 'male'];
  
  let selected = esVoices.find(v => {
    const name = v.name.toLowerCase();
    return maleNames.some(m => name.includes(m));
  });
  
  if (!selected) {
    selected = esVoices.find(v => {
      const name = v.name.toLowerCase();
      return !name.includes('sabina') && !name.includes('monica') && !name.includes('paulina') && !name.includes('female');
    });
  }
  
  return selected || esVoices[0] || voices[0];
}

function hablarNotificacionTexto(texto) {
  if (!('speechSynthesis' in window)) return;
  try {
    window.speechSynthesis.cancel();
    const clean = texto.replace(/[^\w\sÁÉÍÓÚáéíóúÑñ0-9:,.!\-]/gi, '').trim();
    if (!clean) return;
    ...
    try {
      ...
      if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata = new MediaMetadata({
          title: 'Voz de Leónidas - ICDE',
          artist: 'Rey Leónidas Inmobiliaria'
        });
      }
    } catch(e) {}
    ...
    const leonidasVoice = obtenerVozLeonidas(voices);
    if (leonidasVoice) {
      utterance.voice = leonidasVoice;
      utterance.lang = leonidasVoice.lang;
    }
    
    const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
    
    utterance.rate = 0.82;
    utterance.pitch = isMobile ? 0.72 : 0.52;
    utterance.volume = 1.0;
    
    window.speechSynthesis.speak(utterance);
  } catch(e) {
    console.error('Error al hablar voz de Leónidas:', e);
  }
}
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'SPEAK_TEXT') {
      hablarNotificacionTexto(event.data.text);
    }
  });
}

function enviarNotificacionSistema(titulo, cuerpo, datos = {}) {
...
function parseFechaISO(fechaStr, horaStr) {
  ...
  let h = 0, min = 0;
  if (horaStr) {
    const timeParts = String(horaStr).trim().split(':').map(Number);
...
async function verificarAlertasProgramadasCRM() {
  ...
  // -------------------------------------------------------------
  // 1. REVISIÓN DE CITAS (30 minutos antes - Programación en Nube y Local)
  // -------------------------------------------------------------
  if (typeof citas !== 'undefined' && Array.isArray(citas)) {
    citas.forEach(cita => {
      ...
      const citaDate = parseFechaISO(cita.fecha, cita.hora);
      if (!citaDate || isNaN(citaDate.getTime())) return;

      const alertTime = new Date(citaDate.getTime() - 30 * 60 * 1000); // 30 minutos antes de la cita
      const idCode = cita.id || cita.codigo || 'cita';
      const schedKey = `cita_sched_${idCode}_${cita.fecha}_${cita.hora}`;
      const firedKey = `cita_fired_${idCode}_${cita.fecha}_${cita.hora}`;
      ...
      // CASO A: Programar la cita futura (faltan entre 1 minuto y 24 horas para los 30 min antes)
      if (diffMs > 0 && diffMs <= 24 * 3600 * 1000) {
        if (!sentMem[schedKey]) {
          guardarAlertaEnviadaMemoria(schedKey);

          // 1. Programar temporizador diferido en Service Worker del celular
          if ('serviceWorker' in navigator) {
            navigator.serviceWorker.ready.then(reg => {
              if (reg && reg.active) {
                reg.active.postMessage({
          ...
          // 2. Programar notificación en la NUBE OneSignal
          programarWebPushNube(titulo, cuerpo, alertTime, schedKey);
        }
      }

      // CASO B: Disparar la alerta si el momento actual está dentro de los 35 minutos previos a la cita
      const citaDiffMins = Math.round((citaDate.getTime() - now.getTime()) / 60000);
      if (citaDiffMins >= 0 && citaDiffMins <= 35) {
        if (!sentMem[firedKey]) {
          guardarAlertaEnviadaMemoria(firedKey);
          enviarWebPushDirecto(titulo, cuerpo);
          enviarNotificacionSistema(titulo, cuerpo, { tag: firedKey });
        }
      }
    });
  }
...
async function refreshData() {
  ...
  await Promise.all([cargarProps(), cargarKmz(), cargarLeads()]);
  rebuildCitas();
...
async function fetchJsonSeguro(url) {
  ...
  if (text.trim().startsWith('<')) {
    throw new Error('Respuesta HTML de Google Apps Script (Servidor no publicado o 404)');
  }
  return JSON.parse(text);
}

const autoSyncLeads = (...args) => autoSyncDrive(...args);
...
async function autoSyncDrive() {
  ...
  try {
    const [data, dataCitas] = await Promise.all([
      fetchJsonSeguro(CRM_SCRIPT_URL + '?action=getLeads&t=' + Date.now()),
      fetchJsonSeguro(CRM_SCRIPT_URL + '?action=getCitas&t=' + Date.now())
    ]);
    ...
    console.warn('[AutoSync] Error:', e.message || e);
...
async function cargarLeads(){
  ...
  } catch(e){ console.error("Error cargando leads:", e.message || e); }
}
...
function rebuildCitas() {
  ...
  citas = allVisitas;
  setTimeout(verificarAlertasProgramadasCRM, 1000);
}
...
async function cargarProps(){
  try {
    try { localStorage.removeItem('icde_cached_admin_props'); } catch(e) {}
    const st = document.getElementById('syncStatus');
    if (st) st.textContent = '⟳ Cargando datos en vivo...';

    let data = null;
    try {
      const urlBusca = APPS_SCRIPT_URL + "&t=" + Date.now();
      const resBusca = await fetch(urlBusca, { cache: 'no-store' });
      if (resBusca.ok) {
        data = await resBusca.json();
      }
    } catch(e) {
      console.warn('Fallback a datos_catalogo.json:', e);
    }
    ...
    const stSuccess = document.getElementById('syncStatus');
    if (stSuccess) stSuccess.textContent=`✓ ${allProps.length} propiedades`;
    ...
    const stErr = document.getElementById('syncStatus');
    if (stErr) stErr.textContent='⚠ Sin conexión'; 
    allProps=[];
  }
...
function toggleNlData(){
  ...
  // Animar sin re-renderizar toda la página
  const layout = document.querySelector('.nuevo-lead-layout');
  const btn    = document.querySelector('.btn-datos-bottom');
  if(layout){
    if(nlShowData){
      layout.classList.remove('hide-data');
      layout.classList.add('nl-data-open');
    } else {
      layout.classList.add('hide-data');
      layout.classList.remove('nl-data-open');
    }
  }
  if(btn){
    const lbl = btn.querySelector('.btn-datos-lbl');
    if(lbl) lbl.textContent = nlShowData ? '📤 Enviar propuestas' : '📤 Enviar propuestas';
    // El chevron se anima via CSS (.nl-data-open .btn-datos-chevron)
  }
}
...
function renderNuevo(){
  ...
  c.innerHTML=`
  <div class="nuevo-lead-layout ${nlShowData ? 'nl-data-open' : 'hide-data'}">
    <!-- COLUMNA 1: FILTROS E INMUEBLES (30%) -->
    <div class="nl-col-1">
      <div class="panel-section">
        <div class="form-section-header" style="display:flex; justify-content:space-between; align-items:center; margin-top:12px; margin-bottom:10px; min-height:32px;">
          <div class="panel-section-title" style="margin:0 !important; display:inline-flex; align-items:center; line-height:1; height:32px;">Filtros</div>
          <button class="btn-nueva-cita-premium" onclick="showTab('gestion')" style="height:32px; font-size:11.5px; padding:0 12px; border-radius:8px; display:inline-flex; align-items:center; justify-content:center; margin:0;">
            <span>+ Agregar inmuebles</span>
          </button>
        </div>
        <div class="section-content">
          <p class="filtros-subtitle" style="font-size:13px;color:var(--muted);margin-bottom:12px;">Selecciona los filtros para encontrar las propiedades ideales automáticamente.</p>
          ${allProps.length ? `<div id="nlFiltrosWrap"></div>` : `<div class="loading"><div class="spinner"></div> Cargando catálogo...</div>`}
        </div>

        <!-- COLUMNA DATOS Y PLAN (DESPUÉS DE div.pc-buscar-row) -->
        <div class="nl-col-2" style="margin-top:16px;">
          <div class="panel-section" style="border-radius:16px; padding:20px; margin-bottom:16px;">
            <!-- BLOQUE 1: DATOS -->
            ...
            <div class="section-content">
              ...
              <div class="form-grid">
                ...
                <div class="form-row-2col" style="display:grid; grid-template-columns:1fr 1fr; gap:10px; width:100%; grid-column:1/-1;">
        ...
        <div class="form-section-header" style="border-top:1px solid rgba(255,255,255,0.05); padding-top:16px; display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
          <div>
            <div class="panel-section-title" style="margin:0 !important;">Selección de Propiedades</div>
            <span style="font-size:12px;color:#888888; font-weight:500; display:block; margin-top:4px; margin-left:14px;" id="selCnt">${selectedProps.length} seleccionadas</span>
          </div>
...
function getFrecInfo(){
  let f = filtrarProps(nuevoLead.filtros || {}, true);
  if(typeof criterioOrden !== 'undefined'){
    if(criterioOrden==='destacadas') f = f.filter(a => ["Directo", "Verbal"].includes(a["Contrato"]));
    else if(criterioOrden==='noUbicadas') f = f.filter(p => {
      const lat = parseFloat(String(p['Latitud'] || p['Lat'] || '').replace(',','.'));
      const lng = parseFloat(String(p['Longitud'] || p['Lng'] || '').replace(',','.'));
...
let modalTipoLead = 'cliente';

function abrirModalNuevoLead() {
  modalTipoLead = 'cliente';
  const inputN = document.getElementById('mnlN');
  const inputC = document.getElementById('mnlC');
  const inputNI = document.getElementById('mnlNI');
  const inputNA = document.getElementById('mnlNA');
  const inputNotas = document.getElementById('mnlNotas');
  const selEstado = document.getElementById('mnlEstado');
  const selEtiqueta = document.getElementById('mnlEtiqueta');
  const selFrecuencia = document.getElementById('mnlFrecuencia');
  const warn = document.getElementById('modalNlDuplicateWarning');

  if (inputN) inputN.value = '';
  if (inputC) inputC.value = '';
  if (inputNI) inputNI.value = '';
  if (inputNA) inputNA.value = '';
  if (inputNotas) inputNotas.value = '';
  if (selEstado) selEstado.value = 'enviando';
  if (selEtiqueta) selEtiqueta.value = 'activo';
  if (selFrecuencia) selFrecuencia.value = 'semanal';
  if (warn) warn.style.display = 'none';

  setTipoModalLead('cliente');

  const modal = document.getElementById('nuevoLeadModal');
  if (modal) {
...
function setTipoModalLead(tipo) {
  modalTipoLead = tipo;
  const btnC = document.getElementById('modalBtnTC');
  const btnI = document.getElementById('modalBtnTI');
  const inmobExtra = document.getElementById('modalInmobExtra');
  if (tipo === 'cliente') {
    if (btnC) btnC.className = 'btn btn-primary';
    if (btnI) btnI.className = 'btn btn-secondary';
    if (inmobExtra) inmobExtra.style.display = 'none';
  } else {
    if (btnC) btnC.className = 'btn btn-secondary';
    if (btnI) btnI.className = 'btn btn-gold';
    if (inmobExtra) inmobExtra.style.display = 'block';
  }
}

function checkDuplicateLeadModal() {
  const nom = (document.getElementById('mnlN')?.value || '').trim();
  const cel = (document.getElementById('mnlC')?.value || '').trim();
  const warn = document.getElementById('modalNlDuplicateWarning');
  if (!warn) return;
  ...
  const dup = leads.find(l => {
    ...
    const matchCel = celClean && lCel && (celClean.length >= 10 && lCel.length >= 10 ? celClean.slice(-10) === lCel.slice(-10) : celClean === lCel);
    const matchNom = nom && l.nombre && typeof norm === 'function' && norm(l.nombre) === norm(nom);
    return matchCel || matchNom;
  });

  if (dup) {
    warn.style.display = 'block';
    warn.innerHTML = `⚠️ <b>Atención:</b> Ya existe un cliente con datos similares: <b>${dup.nombre}</b> (${dup.celular}).`;
  } else {
    warn.style.display = 'none';
  }
}

function guardarModalLead(irACatalogo, btn) {
  const nom = (document.getElementById('mnlN')?.value || '').trim();
  const cel = (document.getElementById('mnlC')?.value || '').trim();
  const nomInmob = (document.getElementById('mnlNI')?.value || '').trim();
  const nomAgente = (document.getElementById('mnlNA')?.value || '').trim();
  const estado = document.getElementById('mnlEstado')?.value || 'enviando';
  const etiqueta = document.getElementById('mnlEtiqueta')?.value || 'activo';
  const frecuencia = document.getElementById('mnlFrecuencia')?.value || 'semanal';
  const notas = (document.getElementById('mnlNotas')?.value || '').trim();

  if (!nom && modalTipoLead !== 'inmobiliaria') { toast('Ingresa el nombre completo', 'error'); return; }
  if (modalTipoLead === 'inmobiliaria' && !nomInmob) { toast('Ingresa nombre de la inmobiliaria', 'error'); return; }
  if (!cel) { toast('Ingresa el celular', 'error'); return; }
  ...
  if (existe) {
    toast(`Ya existe un cliente con este nombre o celular (${existe.nombre})`, 'error');
    return;
  }
  ...
  const lead = {
    id: 'U-' + Date.now() + Math.floor(Math.random() * 1000),
    tipo: modalTipoLead,
    nombre: modalTipoLead === 'inmobiliaria' ? (nom || nomInmob) : nom,
    celular: cel,
    notas: notas,
    metodoPago: [],
    estado: estado,
    ...
    maxPorEnvio: 4,
    nombreInmobiliaria: nomInmob,
    nombreAgente: nomAgente,
    propsFiltradas: [],
    propsEnviadas: [],
    proximosEnvios: typeof generarProximosCheck === 'function' ? generarProximosCheck(frecuencia) : [],
    historialEnvios: [],
    visitas: [],
  ...
  leads.push(lead);
  saveLeads();
  toast('Cliente guardado con éxito ✓', 'success');
  closeModal('nuevoLeadModal');
  ...
  if (irACatalogo) {
    nuevoLead = {
      ...
      nombreInmobiliaria: lead.nombreInmobiliaria,
...
function renderLeads(){
  const cntTodos = leads.length;
  const cntActivos = leads.filter(l => l.etiqueta === 'activo').length;
  const cntTibios = leads.filter(l => l.etiqueta === 'tibio').length;
  const cntInactivos = leads.filter(l => l.etiqueta === 'inactivo').length;
  const cntInmobiliarias = leads.filter(l => l.tipo === 'inmobiliaria').length;

  document.getElementById('mainContent').innerHTML=`
  <!-- PANEL DE CONTROLES DE LEADS -->
  <div class="panel-section leads-header-panel-section" style="padding: 4px 4px; background: rgba(255,255,255,0.04); border: none; border-radius: 16px; box-shadow: 3px 3px 6px rgba(0,0,0,0.4), inset 1px 1px 0px rgba(255,255,255,0.1), inset -1px -1px 0px rgba(0,0,0,0.3); margin: 4px 0px 4px;">
    <div style="display:flex; justify-content:space-between; align-items:center; gap: 10px; flex-wrap: wrap; width: 100%; padding: 0 4px; box-sizing: border-box; row-gap: 12px;">
      <!-- FILTROS (IZQUIERDA) -->
      <div class="chips-row-container" style="display:flex; gap:4px; flex-wrap: nowrap; align-items: center; overflow-x: auto; max-width: 100%; -webkit-overflow-scrolling: touch; scrollbar-width: none;">
        <button class="chip-filtro ${currentLeadFilter==='todos'?'sel':''}" id="fA" onclick="filtTabla('todos',this)">Todos (${cntTodos})</button>
        <button class="chip-filtro ${currentLeadFilter==='activo'?'sel':''}" id="fAc" onclick="filtTabla('activo',this)">🟢 Activos (${cntActivos})</button>
        <button class="chip-filtro ${currentLeadFilter==='tibio'?'sel':''}" id="fTi" onclick="filtTabla('tibio',this)">🟡 Tibios (${cntTibios})</button>
        <button class="chip-filtro ${currentLeadFilter==='inactivo'?'sel':''}" id="fIn" onclick="filtTabla('inactivo',this)">⚫ Inactivos (${cntInactivos})</button>
        <button class="chip-filtro ${currentLeadFilter==='inmobiliaria'?'sel':''}" id="fIm" onclick="filtTabla('inmobiliaria',this)">🏢 Inmobiliarias (${cntInmobiliarias})</button>
      </div>

      <!-- BUSCADOR Y ACCIONES (DERECHA) -->
      <div class="leads-controls-right" style="display:flex; align-items:center; gap:8px; justify-content: flex-end; flex-wrap: nowrap; min-width: 0;">
        <div class="leads-search-row">
          <div class="pc-buscar-row-premium" style="margin-bottom: 0; flex-shrink: 1; min-width: 0;">
          ...
          <button class="btn-nueva-cita-premium" onclick="abrirModalNuevoLead()" style="height: 38px; border-radius: 8px; flex-shrink: 0; white-space: nowrap;">
...
function renderLeadsBody(lista){
  ...
  const oldWrap = container.querySelector('.leads-table-wrap');
  const prevScrollLeft = oldWrap ? oldWrap.scrollLeft : 0;
  const prevScrollTop = oldWrap ? oldWrap.scrollTop : 0;
  const mainContent = document.getElementById('mainContent');
  const prevMainScrollTop = mainContent ? mainContent.scrollTop : 0;
  ...
  if(leadViewMode === 'table'){
    container.innerHTML=`
    <div class="leads-table-wrap panel-section" style="padding:0; background: rgba(255,255,255,0.04); border: none; border-radius: 16px; box-shadow: 3px 3px 6px rgba(0,0,0,0.4), inset 1px 1px 0px rgba(255,255,255,0.1), inset -1px -1px 0px rgba(0,0,0,0.3); margin-top: 0; overflow-x: auto;">
  ...
  // Restaurar posición de scroll guardada (horizontal y vertical)
  const newWrap = container.querySelector('.leads-table-wrap');
  if (newWrap) {
    newWrap.scrollLeft = prevScrollLeft;
    newWrap.scrollTop = prevScrollTop;
  }
  if (mainContent && prevMainScrollTop) {
    mainContent.scrollTop = prevMainScrollTop;
  }
}
...
function renderCitas(){
  document.getElementById('mainContent').innerHTML=`
    <!-- PANEL DE CONTROLES -->
    <div class="panel-section citas-header-panel-section" style="padding: 4px 4px; background: rgba(255,255,255,0.04); border: none; border-radius: 16px; box-shadow: 3px 3px 6px rgba(0,0,0,0.4), inset 1px 1px 0px rgba(255,255,255,0.1), inset -1px -1px 0px rgba(0,0,0,0.3); margin: 4px 0px 4px;">
      <div style="display:flex; justify-content:space-between; align-items:center; gap: 10px; flex-wrap: wrap; width: 100%; padding: 0 4px; box-sizing: border-box; row-gap: 12px;">
        <!-- FILTROS (IZQUIERDA) -->
        <div class="chips-row-container" style="display:flex; gap:4px; flex-wrap: nowrap; align-items: center; overflow-x: auto; max-width: 100%; -webkit-overflow-scrolling: touch; scrollbar-width: none;">
          <button class="chip-filtro ${citasActiveFilterType === 'hoy' ? 'sel' : ''}" onclick="filterCitas('hoy',this)">Hoy</button>
        ...
        <!-- BUSCADOR Y ACCIONES (DERECHA) -->
        <div class="citas-controls-right" style="display:flex; align-items:center; gap:8px; justify-content: flex-end; flex-wrap: nowrap; min-width: 0;">
          <div class="leads-search-row">
            <div class="pc-buscar-row-premium" style="margin-bottom: 0; flex-shrink: 1; min-width: 0;">
              <span class="bn-icon" style="padding:0 8px 0 12px; color: var(--gold); flex-shrink:0;">
                 <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            ...
            <button class="btn-nueva-cita-premium" onclick="abrirVisitaModal()" style="height: 38px; border-radius: 8px; flex-shrink: 0; white-space: nowrap;">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 4px;"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
              Nueva Cita
            </button>
          </div>
    ...
    <div class="citas-layout-nuevo" style="display:grid; grid-template-columns: 320px 1fr; gap:24px; align-items:start;">
      <div id="calendarContainerWrap" style="display: flex; flex-direction: column; gap: 20px;">
        ...
        <div class="citas-card-top" style="background:var(--bg2); border-radius:20px; padding:20px; box-shadow:3px 3px 0px rgba(0,0,0,0.3),inset 1px 1px 0px rgba(255,255,255,0.15),inset -1px -1px 0px rgba(0,0,0,0.3); width:100%; max-width:100%; box-sizing:border-box; overflow:hidden;">
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px; width:100%; box-sizing:border-box;">
            <div style="width:3px; height:18px; background:var(--gold); border-radius:2px; flex-shrink:0;"></div>
            <span style="font-size:13px; font-weight:700; color:#fff; text-transform:uppercase; letter-spacing:0.08em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">Top Inmuebles más Visitados</span>
          </div>
          <div id="citasTopInmuebles" style="display:flex; flex-direction:column; gap:10px; width:100%; box-sizing:border-box;">
            <div style="color:var(--muted); font-size:13px; text-align:center; padding:20px 0;">Calculando...</div>
          </div>
        </div>

        <!-- VISITAS POR MES -->
        <div class="citas-card-mes" style="background:var(--bg2); border-radius:20px; padding:20px; box-shadow:3px 3px 0px rgba(0,0,0,0.3),inset 1px 1px 0px rgba(255,255,255,0.15),inset -1px -1px 0px rgba(0,0,0,0.3); width:100%; max-width:100%; box-sizing:border-box; overflow:hidden;">
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px; width:100%; box-sizing:border-box;">
            <div style="width:3px; height:18px; background:var(--gold); border-radius:2px; flex-shrink:0;"></div>
            <span style="font-size:13px; font-weight:700; color:#fff; text-transform:uppercase; letter-spacing:0.08em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">Visitas por Mes</span>
          </div>
          <div id="citasVisitasMes" style="display:flex; flex-direction:column; gap:8px; width:100%; box-sizing:border-box;">
            <div style="color:var(--muted); font-size:13px; text-align:center; padding:20px 0;">Calculando...</div>
          </div>
        </div>
      </div>
      <div id="listContainer">
        ...
        <!-- TABLA DE CITAS -->
        <div class="citas-table-wrap panel-section" style="padding: 0; background: rgba(255,255,255,0.04); border: none; border-radius: 12px; box-shadow: 3px 3px 6px rgba(0,0,0,0.4), inset 0 1px 0px rgba(255,255,255,0.15), inset 1px 1px 0px rgba(255,255,255,0.1), inset -1px -1px 0px rgba(0,0,0,0.3); margin-top: 0; overflow-x: auto; max-width: 100%; box-sizing: border-box;">
          <table class="citas-table" style="border: none; border-radius: 0; background: transparent; border-collapse: separate; border-spacing: 0; overflow: visible;">
            <thead>
              <tr style="background: rgba(255, 255, 255, 0.03);">
                <th style="padding-left: 20px; color: var(--gold); border-top: 1px solid rgba(255, 255, 255, 0.12); border-bottom: 1px solid rgba(255, 255, 255, 0.08); box-shadow: inset 0 1px 0px rgba(255, 255, 255, 0.12); position: sticky; top: 0; z-index: 100; background: #161618;">Día / Hora</th>
                <th style="color: var(--gold); border-top: 1px solid rgba(255, 255, 255, 0.12); border-bottom: 1px solid rgba(255, 255, 255, 0.08); box-shadow: inset 0 1px 0px rgba(255, 255, 255, 0.12); position: sticky; top: 0; z-index: 100; background: #161618;">Cliente</th>
                <th style="color: var(--gold); border-top: 1px solid rgba(255, 255, 255, 0.12); border-bottom: 1px solid rgba(255, 255, 255, 0.08); box-shadow: inset 0 1px 0px rgba(255, 255, 255, 0.12); position: sticky; top: 0; z-index: 100; background: #161618;">Inmueble</th>
                <th style="padding-right: 20px; color: var(--gold); border-top: 1px solid rgba(255, 255, 255, 0.12); border-bottom: 1px solid rgba(255, 255, 255, 0.08); box-shadow: inset 0 1px 0px rgba(255, 255, 255, 0.12); position: sticky; top: 0; z-index: 100; background: #161618;">Observaciones</th>
              </tr>
            </thead>
    ...
    <style>
      @media(max-width:900px){ 
        .citas-layout-nuevo { display: flex !important; flex-direction: column !important; gap: 16px !important; width: 100% !important; max-width: 100% !important; min-width: 0 !important; box-sizing: border-box !important; overflow-x: hidden !important; } 
        #calendarContainerWrap { display: contents !important; } 
        #calendarContainer { order: 1 !important; width: 100% !important; max-width: 100% !important; box-sizing: border-box !important; }
        #listContainer { order: 2 !important; width: 100% !important; max-width: 100% !important; min-width: 0 !important; box-sizing: border-box !important; overflow: hidden !important; } 
        .citas-table-wrap { width: 100% !important; max-width: 100% !important; overflow-x: auto !important; box-sizing: border-box !important; }
        .citas-card-top { order: 97 !important; width: 100% !important; max-width: 100% !important; box-sizing: border-box !important; padding: 14px 12px !important; border-radius: 14px !important; overflow: hidden !important; margin: 0 !important; }
        .citas-card-mes { order: 98 !important; width: 100% !important; max-width: 100% !important; box-sizing: border-box !important; padding: 14px 12px !important; border-radius: 14px !important; overflow: hidden !important; margin: 0 !important; }
      }
...
function renderTopVisitados() {
  ...
  container.innerHTML = sorted.map(([cod, count], i) => {
    ...
    return `
      <div style="display:flex; align-items:center; gap:8px; background:rgba(255,255,255,0.03); border-radius:12px; padding:8px 10px; box-shadow:2px 2px 0px rgba(0,0,0,0.3),inset 1px 1px 0px rgba(255,255,255,0.08); width:100%; box-sizing:border-box; overflow:hidden;">
        <span style="font-size:11px; font-weight:800; color:${rankColors[i]}; width:14px; text-align:center; flex-shrink:0;">#${i+1}</span>
        <img src="${img}" onerror="this.src='https://i.imgur.com/Pc9M3I8.png'" style="width:38px; height:38px; border-radius:8px; object-fit:cover; flex-shrink:0;"/>
        <div style="flex:1; min-width:0; overflow:hidden;">
          <div style="font-size:12px; font-weight:700; color:var(--gold); line-height:1.2; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${cod}</div>
          <div style="font-size:11.5px; color:#ccc; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${nombre}</div>
          <div style="margin-top:4px; height:3px; background:rgba(255,255,255,0.07); border-radius:2px; overflow:hidden;">
        ...
        <div style="font-size:12px; font-weight:800; color:var(--green); flex-shrink:0; white-space:nowrap;">${count} <span style="font-size:9.5px; font-weight:500; color:var(--muted);">vis.</span></div>
      </div>`;
  }).join('');
}
...
function renderVisitasPorMes() {
  ...
  // Tarjeta de total
  const totalHtml = `
    <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.03); border-radius:12px; padding:10px 14px; margin-bottom:4px; box-shadow:inset 2px 2px 4px rgba(0,0,0,0.4); width:100%; box-sizing:border-box; overflow:hidden;">
      <div>
        <div style="font-size:11px; color:var(--muted); text-transform:uppercase; letter-spacing:0.1em;">Total registrado</div>
        <div style="font-size:24px; font-weight:800; color:#fff; line-height:1.1; margin-top:2px;">${totalVisitas}</div>
        <div style="font-size:11px; color:var(--gold); font-weight:600; margin-top:2px;">Histórico acumulado</div>
      </div>
      <div style="font-size:26px; opacity:0.15; flex-shrink:0;">📅</div>
    </div>`;
  ...
  const barsHtml = entries.map(e => {
    ...
    return `
      <div style="display:flex; align-items:center; gap:8px; width:100%; box-sizing:border-box;">
        <div style="font-size:11px; color:var(--muted); width:32px; flex-shrink:0; text-align:right;">${e.label}</div>
        <div style="flex:1; height:20px; background:rgba(255,255,255,0.05); border-radius:4px; overflow:hidden; position:relative; min-width:0;">
  ...
  container.innerHTML = totalHtml + `<div style="display:flex; flex-direction:column; gap:6px; margin-top:12px; width:100%; box-sizing:border-box;">${barsHtml}</div>`;
}
...
function renderCitasBody(lista){
  ...
  if (!lista || lista.length === 0) {
    let msg = 'No hay citas agendadas para hoy';
    if (citasActiveFilterType === 'semana') msg = 'No hay citas agendadas para esta semana';
    else if (citasActiveFilterType === 'mes') msg = 'No hay citas agendadas para este mes';
    else if (citasActiveFilterType === 'fecha') msg = `No hay citas agendadas para la fecha ${citasActiveFilterValue}`;
    else if (citasActiveFilterType === 'todas') msg = 'No hay citas registradas';

    b.innerHTML = `<tr>
      <td colspan="4" style="text-align:center; padding: 35px 15px; color: var(--muted); font-size:13px; background: rgba(0,0,0,0.1); word-break: break-word; white-space: normal; width: 100%; box-sizing: border-box;">
        <div style="font-size:32px; margin-bottom:10px;">📅</div>
        <div style="font-weight:700; color:#fff; font-size:15px; margin-bottom:6px;">${msg}</div>
        <div style="font-size:12px; color:#aaa; line-height:1.4;">Puedes hacer clic en <b>"Semana"</b>, <b>"Mes"</b> o <b>"Todas"</b> arriba para ver las citas de otras fechas.</div>
      </td>
    </tr>`;
    return;
  }
  ...
  const ordenada = [...lista].sort((a,b) => {
    ...
    const isoA = getFechaISO(a.fecha) || a.fecha || '';
    const isoB = getFechaISO(b.fecha) || b.fecha || '';
    const da = isoA + (a.hora ? 'T' + a.hora : 'T00:00');
    const db = isoB + (b.hora ? 'T' + b.hora : 'T00:00');
    return da.localeCompare(db);
  });

  b.innerHTML = ordenada.map(c => {
    const isoFecha = getFechaISO(c.fecha);
    const d = new Date((isoFecha || c.fecha) + 'T12:00:00');
    ...
    return `<tr class="clickable ${isSelected ? 'active-cita-row' : ''}" data-cita-id="${String(c.id).trim()}" onclick="abrirEditarCita('${c.id}')">
      <td class="col-citas-fecha" style="padding-left: 20px;"><div class="citas-time">${c.hora || '--:--'}</div><div class="citas-weekday">${isNaN(d.getDay()) ? '—' : diasSem[d.getDay()]} ${isoFecha || c.fecha}</div></td>
      <td class="col-citas-cliente">
        <div class="citas-cliente-flex" style="display:flex; justify-content:space-between; align-items:flex-start;">
          <div>
        ...
        <div class="citas-cliente-inmob" style="font-size:10px;color:var(--gold);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:150px;" title="${c.inmobiliaria||'Directo'}">🏢 ${c.inmobiliaria || 'Directo'}</div>
      </td>
      <td class="col-citas-inmueble">
        <div style="font-weight:600;color:var(--gold);">${c.codigo}</div>
        <div class="citas-inm-sub" style="font-size:10px;color:#888;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:90px;" title="${inmobProp}">${inmobProp || '—'}</div>
      </td>
      <td class="col-citas-obs" style="padding-right: 20px; font-size:12px;color:#aaa;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="${c.observaciones||c.notas||''}">${c.observaciones || c.notas || ''}</td>
    </tr>`;
  }).join('');
}
...
function filterCitas(tipo, btn){
  ...
  const elSelFecha = document.getElementById('citasSelFecha');
  if (elSelFecha) {
    elSelFecha.textContent = tipo === 'hoy' ? 'Mostrando citas de hoy' : '';
  }
...
function aplicarFiltroYBusquedaCitas(){
  ...
  const localToday = new Date(hoy.getTime() - (offset * 60 * 1000)).toISOString().slice(0, 10);
  let filtered = citas;

  if (citasActiveFilterType === 'hoy') {
    filtered = citas.filter(c => getFechaISO(c.fecha) === localToday);
  } else if (citasActiveFilterType === 'semana') {
    const d1 = new Date(hoy);
    const start = new Date(d1.setDate(d1.getDate() - d1.getDay()));
    start.setHours(0, 0, 0, 0);
    const startISO = new Date(start.getTime() - (offset * 60 * 1000)).toISOString().slice(0, 10);
    
    const d2 = new Date(hoy);
    const end = new Date(d2.setDate(d2.getDate() - d2.getDay() + 6));
    end.setHours(23, 59, 59, 999);
    const endISO = new Date(end.getTime() - (offset * 60 * 1000)).toISOString().slice(0, 10);

    filtered = citas.filter(c => {
      const iso = getFechaISO(c.fecha);
    ...
    filtered = citas.filter(c => {
      const iso = getFechaISO(c.fecha);
      return iso && iso.startsWith(targetMonth);
    });
  } else if (citasActiveFilterType === 'fecha') {
    filtered = citas.filter(c => getFechaISO(c.fecha) === citasActiveFilterValue);
  }

  if (searchTermCitas) {
    const normalizedTerm = norm(searchTermCitas);
    filtered = filtered.filter(c => norm(c.cliente || '').includes(normalizedTerm) || norm(c.celular || '').includes(normalizedTerm) || norm(c.codigo || '').includes(normalizedTerm));
  }
...
function abrirSettings(){
  ...
  const oneSignalInput = document.getElementById('setOneSignalApiKey');
  ...
  if (oneSignalInput)  oneSignalInput.value  = settings.oneSignalApiKey || '';
...
function guardarSettings(){
  ...
  if (document.getElementById('setOneSignalApiKey')) {
    settings.oneSignalApiKey = document.getElementById('setOneSignalApiKey').value.trim();
  }
...
function speakTextBrowserFallback(cleanText) {
  if ('speechSynthesis' in window) {
    ...
    const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
    utterance.rate = 0.82; // Cadencia imponente de Leónidas
    utterance.pitch = isMobile ? 0.72 : 0.52; // Tono de comandante espartano
    utterance.volume = 1.0;
...
function toggleGemini() {
  ...
  if (isGeminiListening) {
    ...
    toggleGeminiSidebar();
    if (!isGeminiListening) {
      setTimeout(() => toggleGeminiMic(), 200);
    }
  }
}
...
function toggleGeminiMic() {
  ...
  if (isGeminiListening) {
    ...
    recognition.onstart = () => {
      ...
      showGeminiSpeechBubble("🎙️ <strong>Escuchando...</strong> Habla ahora.", 0);
      finalTranscript = '';
    ...
    data.properties.forEach(p => {
      ...
      if (p.payments) {
        ...
        chronologicalMonths.forEach((item, idx) => {
          ...
          if (valUpper.includes('DESOCUPAD') || m.status === 'VACANT') {
            m.status = 'VACANT';
            m.value = 'DESOCUPADO';
          } else if (isPaidCell) {
...
function mergeLocalAdminData(freshData) {
  ...
  // Preservar owner_notes, improvements, quotes del caché local si el backend aún no los devuelve
  const localCached = JSON.parse(localStorage.getItem('icde_admin_data') || '{}');
  const localProps  = Array.isArray((localCached||{}).properties) ? localCached.properties : [];
  freshData.properties.forEach(p => {
    ...
    // Preservar campos de propietario desde local si el backend no los trae
    const localP = localProps.find(lp => String(lp.id) === String(p.id));
    if (localP) {
      if (!Array.isArray(p.owner_notes)  && Array.isArray(localP.owner_notes))  p.owner_notes  = localP.owner_notes;
      if (!Array.isArray(p.improvements) && Array.isArray(localP.improvements)) p.improvements = localP.improvements;
      if (!Array.isArray(p.quotes)       && Array.isArray(localP.quotes))       p.quotes       = localP.quotes;
    }
    if (!Array.isArray(p.owner_notes))  p.owner_notes  = [];
    if (!Array.isArray(p.improvements)) p.improvements = [];
    if (!Array.isArray(p.quotes))       p.quotes       = [];
  });
  return freshData;
}

async function loadAdminData(showToastNotice = false) {
  try {
    console.log('Cargando datos de administración directamente desde Google Drive...');
    ...
    if (res.ok) {
      ...
      if (data && !data.error && data.properties && data.properties.length) {
        ...
        console.log('Datos de administración cargados desde Google Drive con éxito.');
        if (showToastNotice && typeof toast === 'function') {
          toast('¡Datos sincronizados en vivo desde Google Drive!', 'success');
        }
        return true;
      }
    }
  } catch (e) {
    console.warn('Error al cargar desde Google Drive:', e);
  }

  // Fallback: admin_data.json estático local si falla la red
  try {
...
function renderAdministracion() {
  ...
  c.innerHTML = `
    ...
    <div class="panel-section admin-header-panel-section" style="padding: 4px 4px; background: rgba(255,255,255,0.04); border: none; border-radius: 16px; box-shadow: 3px 3px 6px rgba(0,0,0,0.4), inset 1px 1px 0px rgba(255,255,255,0.1), inset -1px -1px 0px rgba(0,0,0,0.3); margin: 4px 0px 4px;">
      <div class="admin-panel-controls-inner" style="display:flex; justify-content:space-between; align-items:center; gap: 10px; flex-wrap: wrap; row-gap: 10px; width: 100%; padding: 0 20px; box-sizing: border-box;">
        
        <!-- FILA DE BÚSQUEDA Y ORDENACIÓN -->
        <div class="admin-search-sort-row">
          <!-- BUSCADOR (IZQUIERDA) -->
          <div class="pc-buscar-row-premium admin-search-wrap" style="max-width: 320px; margin-bottom: 0; flex-shrink: 1; min-width: 140px;">
          ...
          <div class="admin-sort-wrap" style="position: relative; display: inline-block; flex-shrink: 0; cursor: pointer;">
            <div class="btn-nueva-cita-premium" style="display: flex; align-items: center; justify-content: space-between; height: 38px; padding: 0 12px; font-family: 'Outfit', sans-serif; font-size: 13px; color: #C0C0C0; font-weight: 700; cursor: pointer; pointer-events: none; gap: 8px;">
              <span>Ordenar por</span>
              <span style="color: var(--gold); font-size: 10px; display: inline-flex; align-items: center;">▼</span>
            </div>
        ...
        <div id="adminFiltersContainer" style="display: flex; gap: 8px; align-items: center; flex-shrink: 0; flex-wrap: wrap;">
          ...
          <button class="chip-filtro ${currentAdminFilter==='testimonios'?'sel':''}" data-filter="testimonios" onclick="setAdminFilter('testimonios')">📋 Testimonios</button>
        </div>

        <!-- ACCIONES Y FILTROS (DERECHA) -->
        <div class="admin-actions-wrap" style="display: flex; gap: 10px; align-items: center; flex-shrink: 0; flex-wrap: wrap; min-width: 0; row-gap: 8px;">
          <span style="font-size: 11px; color: var(--muted); font-weight: 700; letter-spacing:0.05em; flex-shrink: 0;">AÑO:</span>
          <select id="adminYearSelect" class="btn-nueva-cita-premium" style="min-width: 95px; width: 95px; height: 38px; padding: 0 20px 0 12px; font-size: 13px; color: #C0C0C0 !important; font-weight: 700; cursor:pointer; flex-shrink: 0; color-scheme: dark; box-sizing: border-box; font-family: 'Outfit', sans-serif;" onchange="changeAdminYear(this.value)">
            ...
            <option value="2028" style="background-color: #121212; color: #fff;" ${currentAdminYear === '2028' ? 'selected' : ''}>2028</option>
            <option value="2029" style="background-color: #121212; color: #fff;" ${currentAdminYear === '2029' ? 'selected' : ''}>2029</option>
            <option value="2030" style="background-color: #121212; color: #fff;" ${currentAdminYear === '2030' ? 'selected' : ''}>2030</option>
          </select>
          <button class="btn-nueva-cita-premium" style="height: 38px; flex-shrink: 0; padding: 0 14px; font-weight: 700; color: var(--gold) !important;" onclick="abrirCrearInmuebleAdmin()"><span style="color: var(--gold); font-size: 14px; margin-right: 4px;">+</span> <span style="color: var(--gold);">Crear Nueva Propiedad</span></button>
        </div>
      </div>
    </div>
...
function renderAdministracionContent() {
  ...
  // ── Vista especial: Testimonios ──────────────────────────────────────
  if (currentAdminFilter === 'testimonios') {
    renderAdminTestimonios(tabContent);
    return;
  }
      ...
      <table class="admin-table">
        ...
        <tbody>
          ${propertiesList.map(p => {
            ...
            return `
              <tr onclick="openMatrixCellDetails('${p.id}', '${currentAdminMonth}', '${currentAdminYear}')" style="cursor: pointer;">
                <td>
                ...
                <td>
                  ...
                  <div style="font-size:11px; color:var(--muted); margin-top:2px;">
                    📅 Inicio: ${p.start_date || 'N/A'} | ⏳ Fin: <strong style="color:var(--gold);">${calculateAdminEndDate(p.start_date, p.duration) || 'N/A'}</strong> (${p.duration || 12} meses)
                  </div>
                </td>
                <td>
      ...
      <table class="matrix-table">
        ...
        <tbody>
          ${propertiesList.map(p => {
            ...
            return `
              <tr class="${isSelected ? 'active-admin-row' : ''}" data-prop-name="${String(p.name).trim()}" onclick="openMatrixCellDetails('${p.id}', '${currentAdminMonth}', '${currentAdminYear}')" style="cursor: pointer;">
                <td class="cell-prop" title="${p.name} (Propietario: ${p.owner}${p.tenant_name ? `, Inquilino: ${p.tenant_name}` : ''})" onclick="openMatrixCellDetails('${p.id}', '${currentAdminMonth}', '${currentAdminYear}')" style="cursor: pointer;">
                ...
                ${monthsFull.map(mFull => {
                  ...
                  // Detectar si este mes es el mes de aniversario/renovación del contrato (soporta 6, 12 meses y cualquier formato de fecha)
                  let isRenovMonth = false;
                  const parsedSDate = parseAdminStartDate(p.start_date);
                  if (parsedSDate && parsedSDate.month >= 1 && parsedSDate.month <= 12 && parsedSDate.year > 0) {
                    ...
                    const startYear = parsedSDate.year;
                    const dur = parseInt(p.duration, 10) || 12;
                    const targetMonthIdx = monthsArray.indexOf(mFull);
                    const viewYear = parseInt(currentAdminYear, 10) || new Date().getFullYear();
                    if (targetMonthIdx !== -1) {
                      const totalMonthsDiff = (viewYear - startYear) * 12 + (targetMonthIdx - startMonthIdx);
                      if (totalMonthsDiff > 0 && (totalMonthsDiff % dur) === 0) {
                        isRenovMonth = true;
                      }
                    }
                  }
                  ...
                  // Si es el mes de renovación y no está en un estado resuelto especial (PAGADO, PREAVISO, ENTREGA, NO RENOVARA, VACANT / DESOCUPADO), marcar como NEW_CONTRACT para mostrar la estrella ★
                  const valUpperPay = String(pay.value || '').trim().toUpperCase();
                  if (isRenovMonth) {
                    if (!["PAID", "PREAVISO", "DELIVERY", "NO_RENEW", "VACANT"].includes(pay.status) && !valUpperPay.includes('DESOCUPAD')) {
                      pay = { ...pay, status: "NEW_CONTRACT" };
                    }
                  } else if (parsedSDate && pay.status === "NEW_CONTRACT") {
                    pay = { ...pay, status: p.tenant_name ? "PENDING" : "VACANT" };
                  }
                  ...
                  const renovBadge = (isRenovMonth && pay.status !== "VACANT" && !valUpperPay.includes('DESOCUPAD')) ? ' | 📝 Mes de Renovación / Término de Contrato' : '';
                  ...
                  return `
                    <td onclick="openMatrixCellDetails('${p.id}', '${mFull}', '${currentAdminYear}')" style="cursor: pointer;">
...
let defaultAdminIpcRate = 5.10;

function roundDownToNearest5k(val) {
  if (!val || val <= 0) return 0;
  return Math.floor(val / 5000) * 5000;
}

function calculateAdminRentInfo(p, customIpcRate) {
  const baseRent = parseFloat(p ? p.monthly_rent : 0) || 0;
  const notes = String(p ? p.increase_notes || '' : '').trim();
  if (!baseRent) return { baseRent: 0, increasedRent: 0, diff: 0, rawIncrease: 0, ipcPct: defaultAdminIpcRate, hasIncrease: false, desc: 'Sin incremento' };

  let ipcPct = customIpcRate !== undefined ? parseFloat(customIpcRate) : defaultAdminIpcRate;
  
  const pctMatch = notes.match(/([0-9]+(?:\.[0-9]+)?)\s*%/);
  if (pctMatch && customIpcRate === undefined) {
    ipcPct = parseFloat(pctMatch[1]);
  }

  const hasOldYearNote = /202[0-5]/.test(notes);
  const dollarMatches = notes.match(/\$\s*([0-9]{1,3}(?:\.[0-9]{3})+|[0-9]+)/g);
  
  if (dollarMatches && dollarMatches.length && !hasOldYearNote && !notes.toLowerCase().includes('ipc')) {
    const lastMatch = dollarMatches[dollarMatches.length - 1];
    const cleanVal = parseInt(lastMatch.replace(/[^0-9]/g, ''), 10);
    if (cleanVal > 0) {
      if (cleanVal < baseRent) {
        const rawRent = baseRent + cleanVal;
        const newRent = roundDownToNearest5k(rawRent);
        const diff = newRent - baseRent;
        return {
          baseRent,
          increasedRent: newRent,
          diff,
          rawIncrease: cleanVal,
          ipcPct,
          hasIncrease: diff > 0,
          desc: `+${safeFormatP(diff)} (Múltiplo de $5.000)`
        };
      } else if (cleanVal > baseRent) {
        const newRent = roundDownToNearest5k(cleanVal);
        const diff = newRent - baseRent;
        return {
          baseRent,
          increasedRent: newRent,
          diff,
          rawIncrease: cleanVal - baseRent,
          ipcPct,
          hasIncrease: diff > 0,
          desc: `Nuevo Canon ${safeFormatP(newRent)} (Múltiplo de $5.000)`
        };
      }
    }
  }

  const rawIncrease = baseRent * (ipcPct / 100.0);
  const roundedIncrease = roundDownToNearest5k(rawIncrease);
  const newRent = baseRent + roundedIncrease;
  const diff = roundedIncrease;
  ...
  return {
    ...
    ipcPct,
    hasIncrease: diff > 0,
    desc: `+${ipcPct}% IPC (IPC: +${safeFormatP(Math.round(rawIncrease))} ➔ Ajustado inquilino: +${safeFormatP(diff)})`
  };
}

function recalculateModalIpc(propId, ipcVal) {
  const p = adminData.properties.find(x => x.id === propId);
  if (!p) return;
  const rate = parseFloat(ipcVal) || 5.10;
  defaultAdminIpcRate = rate;
  const modal = document.getElementById('adminUnifiedModal');
  if (modal && modal.dataset.monthName && modal.dataset.year) {
    const currentTab = document.querySelector('.u-tab-content.active') ? document.querySelector('.u-tab-content.active').id.replace('uTab-', '') : 'cobro';
    modal.innerHTML = _buildUnifiedModalHTML(p, modal.dataset.monthName, modal.dataset.year, currentTab);
  }
}

function applyAdminRentIncrement(propId, newRent) {
  const contraRentInp = document.getElementById('editContraRent');
  const propRentInp = document.getElementById('editPropRent');
  const payValueInp = document.getElementById('editPayValue');
  
  if (contraRentInp) contraRentInp.value = newRent;
  if (propRentInp) propRentInp.value = newRent;
  if (payValueInp) payValueInp.value = newRent;
  
  toast(`¡Fijado nuevo canon de ${safeFormatP(newRent)}! Presione 'Guardar' para sincronizar con Google Drive.`, 'success');
}

function _buildUnifiedModalHTML(p, monthName, year, activeTab = 'cobro') {
  const paymentsYear = p.payments[year] || [];
  const pay = paymentsYear.find(m => m.month === monthName) || { status: 'UNSTARTED', value: 0 };
  const dueDay = p.due_day ? Math.round(p.due_day) : 5;
  const maxDueDay = p.max_due_day ? Math.round(p.max_due_day) : 10;
  const currentActiveTab = activeTab || 'cobro';
  const rentInfo = calculateAdminRentInfo(p);

  const tabs = [
    { id: 'cobro',       icon: '💵', label: 'Cobro y Contrato' },
    { id: 'inmueble',    icon: '🏠', label: 'Inmueble'         },
    { id: 'propietario', icon: '👤', label: 'Propietario'      }
  ];
  ...
  const tabCobro = `
    <div style="display:flex; flex-direction:column; gap:12px;">
      ...
      <div style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.3); border-radius: 8px; padding: 10px 12px; margin-top: -4px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
          <div style="font-size:10.5px; color:#22c55e; font-weight:700; text-transform:uppercase; letter-spacing:0.04em;">
            📈 CÁLCULO DE INCREMENTO IPC (${rentInfo.ipcPct}%)
          </div>
          <div style="display:flex; align-items:center; gap:4px; font-size:11px; color:rgba(255,255,255,0.7);">
            <span>Tasa IPC:</span>
            <input type="number" step="0.01" id="ipcRateInput" value="${rentInfo.ipcPct}" style="width:55px; height:22px; background:#121212; color:#fff; border:1px solid rgba(255,255,255,0.2); border-radius:4px; font-size:11px; text-align:center;" onchange="recalculateModalIpc('${p.id}', this.value)">%
          </div>
        </div>
        <div style="font-size:11.5px; color:rgba(255,255,255,0.85); margin-bottom:6px; line-height:1.4;">
          • Canon Base: <strong>${safeFormatP(p.monthly_rent)}</strong> | Aumento exacto IPC (${rentInfo.ipcPct}%): <strong>+${safeFormatP(Math.round(rentInfo.rawIncrease))}</strong><br>
          • Ajuste a favor del inquilino (múltiplo de $5.000): <strong style="color:#22c55e;">+${safeFormatP(rentInfo.diff)}</strong>
        </div>
        <div style="display:flex; align-items:center; justify-content:space-between; border-top:1px solid rgba(34,197,94,0.2); padding-top:6px;">
          <div>
            <span style="font-size:11px; color:rgba(255,255,255,0.6);">Nuevo Canon Resultante:</span>
            <div style="font-size:16px; font-weight:800; color:#22c55e;">${safeFormatP(rentInfo.increasedRent)}</div>
          </div>
          <button type="button" class="btn btn-xs" style="font-size:11px; font-weight:700; background:#22c55e; color:#000; border:none; padding:5px 12px; border-radius:6px; cursor:pointer;" onclick="applyAdminRentIncrement('${p.id}', ${rentInfo.increasedRent})">
            ⚡ Usar Nuevo Canon (${safeFormatP(rentInfo.increasedRent)})
          </button>
        </div>
      </div>
      ...
      <div style="border-top:1px solid rgba(255,255,255,0.08); padding-top:12px; margin-top:4px; display:flex; flex-direction:column; gap:11px;">
        ...
        <div style="font-size:12px; font-weight:700; color:var(--gold); background:rgba(212,168,75,0.1); border:1px solid rgba(212,168,75,0.3); border-radius:6px; padding:6px 12px; margin-top:-2px; display:flex; align-items:center; gap:6px;">
          🗓️ Fin / Vencimiento de Contrato: <strong>${calculateAdminEndDate(p.start_date, p.duration) || 'N/A'}</strong>
        </div>
  ...
  /* ── TAB 3: PARA EL PROPIETARIO ─────────────────────────────────── */
  const ownerNotes   = Array.isArray(p.owner_notes)  ? p.owner_notes  : [];
  const improvements = Array.isArray(p.improvements) ? p.improvements : [];
  const quotes       = Array.isArray(p.quotes)       ? p.quotes       : [];

  const notesListHTML = ownerNotes.length
    ? ownerNotes.map((n,i) => `
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:8px;padding:10px 12px;margin-bottom:6px;position:relative;">
        <div style="font-size:10px;color:var(--muted);margin-bottom:4px;">${n.date||''}</div>
        <div style="font-size:13px;color:#fff;line-height:1.5;">${(n.text||'').replace(/\n/g,'<br>')}</div>
        <button onclick="ownerRemoveNote('${p.id}',${i})" style="position:absolute;top:6px;right:8px;background:none;border:none;color:rgba(239,68,68,0.6);cursor:pointer;font-size:15px;" title="Eliminar">&times;</button>
      </div>`).join('')
    : '<div style="font-size:12px;color:var(--muted);padding:6px 0;">Sin notas aún.</div>';

  const improvListHTML = improvements.length
    ? improvements.map((imp,i) => `
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:12px;margin-bottom:8px;position:relative;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
          <div style="font-size:13px;font-weight:700;color:#fff;">${imp.title||'Sin título'}</div>
          <select onchange="ownerChangeImprovStatus('${p.id}',${i},this.value)" style="font-size:10px;background:#121212;color:var(--gold);border:1px solid rgba(212,168,75,0.3);border-radius:6px;padding:2px 6px;cursor:pointer;">
            <option value="sugerida"   ${imp.status==='sugerida'   ?'selected':''}>💡 Sugerida</option>
            <option value="en_proceso" ${imp.status==='en_proceso' ?'selected':''}>🔧 En Proceso</option>
            <option value="terminada"  ${imp.status==='terminada'  ?'selected':''}>✅ Terminada</option>
          </select>
        </div>
        ${imp.description?`<div style="font-size:12px;color:var(--muted);margin-bottom:6px;">${imp.description}</div>`:''}
        ${imp.price?`<div style="font-size:12px;color:#22c55e;font-weight:700;margin-bottom:6px;">💲 $${Number(imp.price).toLocaleString('es-CO')}</div>`:''}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:6px;">
          <div>
            <div style="font-size:10px;color:var(--muted);margin-bottom:3px;">📷 ANTES</div>
            ${imp.photo_before_url
              ?`<img src="${imp.photo_before_url}" style="width:100%;height:72px;object-fit:cover;border-radius:6px;cursor:pointer;" onclick="window.open('${imp.photo_before_url}','_blank')" />`
              :`<label style="display:flex;align-items:center;justify-content:center;height:72px;background:rgba(255,255,255,0.03);border:1px dashed rgba(255,255,255,0.15);border-radius:6px;cursor:pointer;font-size:11px;color:var(--muted);">+ Foto<input type="file" accept="image/*" style="display:none" onchange="ownerUploadImprovPhoto('${p.id}',${i},'before',this)"></label>`
            }
          </div>
          <div>
            <div style="font-size:10px;color:var(--muted);margin-bottom:3px;">📷 DESPUÉS</div>
            ${imp.photo_after_url
              ?`<img src="${imp.photo_after_url}" style="width:100%;height:72px;object-fit:cover;border-radius:6px;cursor:pointer;" onclick="window.open('${imp.photo_after_url}','_blank')" />`
              :`<label style="display:flex;align-items:center;justify-content:center;height:72px;background:rgba(255,255,255,0.03);border:1px dashed rgba(255,255,255,0.15);border-radius:6px;cursor:pointer;font-size:11px;color:var(--muted);">+ Foto<input type="file" accept="image/*" style="display:none" onchange="ownerUploadImprovPhoto('${p.id}',${i},'after',this)"></label>`
            }
          </div>
        </div>
        <button onclick="ownerRemoveImprovement('${p.id}',${i})" style="position:absolute;top:8px;right:8px;background:none;border:none;color:rgba(239,68,68,0.6);cursor:pointer;font-size:13px;" title="Eliminar">🗑</button>
      </div>`).join('')
    : '<div style="font-size:12px;color:var(--muted);padding:6px 0;">Sin mejoras registradas.</div>';

  const quotesListHTML = quotes.length
    ? `<table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:4px;">
        <thead><tr>
          <th style="text-align:left;color:var(--muted);padding:4px 6px;font-size:10px;text-transform:uppercase;">Ítem</th>
          <th style="text-align:left;color:var(--muted);padding:4px 6px;font-size:10px;text-transform:uppercase;">Descripción</th>
          <th style="text-align:right;color:var(--muted);padding:4px 6px;font-size:10px;text-transform:uppercase;">Valor</th>
          <th style="width:24px;"></th>
        </tr></thead>
        <tbody>${quotes.map((q,i)=>`
          <tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
            <td style="padding:7px 6px;color:#fff;font-weight:600;">${q.item||''}</td>
            <td style="padding:7px 6px;color:var(--muted);">${q.description||''}</td>
            <td style="padding:7px 6px;color:#22c55e;font-weight:700;text-align:right;">$${Number(q.value||0).toLocaleString('es-CO')}</td>
            <td><button onclick="ownerRemoveQuote('${p.id}',${i})" style="background:none;border:none;color:rgba(239,68,68,0.6);cursor:pointer;font-size:14px;">&times;</button></td>
          </tr>`).join('')}
        </tbody></table>`
    : '<div style="font-size:12px;color:var(--muted);padding:6px 0;">Sin cotizaciones.</div>';

  const tabPropietario = `
    <div style="display:flex;flex-direction:column;gap:0;">
      <div style="font-size:11px;color:var(--gold);font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;">📝 Notas para el Propietario</div>
      <div id="ownerNotesList">${notesListHTML}</div>
      <div style="display:flex;gap:6px;margin-bottom:16px;">
        <textarea id="newOwnerNoteText" class="u-inp" rows="2" style="flex:1;resize:none;" placeholder="Escribe una nota..."></textarea>
        <button onclick="ownerAddNote('${p.id}')" class="btn btn-gold btn-sm" style="height:auto;align-self:stretch;white-space:nowrap;padding:0 12px;">+ Nota</button>
      </div>
      <div style="font-size:11px;color:var(--gold);font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;border-top:1px solid rgba(255,255,255,0.06);padding-top:12px;">🔧 Mejoras / Antes y Después</div>
      <div id="improvementsList">${improvListHTML}</div>
      <button onclick="ownerAddImprovement('${p.id}')" class="btn btn-secondary btn-sm" style="width:100%;margin-bottom:16px;justify-content:center;">+ Nueva Mejora</button>
      <div style="font-size:11px;color:var(--gold);font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;border-top:1px solid rgba(255,255,255,0.06);padding-top:12px;">🛒 Cotizaciones / Tienda</div>
      <div id="quotesList" style="margin-bottom:8px;">${quotesListHTML}</div>
      <div style="display:grid;grid-template-columns:1fr 1fr 90px;gap:6px;margin-bottom:6px;">
        <input type="text" id="newQuoteItem" class="u-inp" placeholder="Ítem" />
        <input type="text" id="newQuoteDesc" class="u-inp" placeholder="Descripción" />
        <input type="number" id="newQuoteValue" class="u-inp" placeholder="$" />
      </div>
      <button onclick="ownerAddQuote('${p.id}')" class="btn btn-secondary btn-sm" style="width:100%;margin-bottom:16px;justify-content:center;">+ Añadir Cotización</button>
      <button id="btnSaveOwnerContent" onclick="saveOwnerContent('${p.id}')" class="btn btn-primary" style="width:100%;justify-content:center;font-weight:700;">
  ...
  return `
    ...
    <div id="uTab-propietario" class="u-tab-content ${currentActiveTab==='propietario' ? 'active' : ''}">${tabPropietario}</div>
  `;
}

/* ═══════════════════════════════════════════════════════════════════
   GESTIÓN CONTENIDO PROPIETARIO — helper functions (Parte A)
═══════════════════════════════════════════════════════════════════ */
function _getOwnerProp(propId) {
  const p = adminData.properties.find(x => x.id === propId);
  if (!p) { if(typeof toast==='function') toast('Propiedad no encontrada','error'); return null; }
  if (!Array.isArray(p.owner_notes))  p.owner_notes  = [];
  if (!Array.isArray(p.improvements)) p.improvements = [];
  if (!Array.isArray(p.quotes))       p.quotes       = [];
  return p;
}
function _refreshOwnerTab(propId) {
  const modal = document.getElementById('adminUnifiedModal');
  if (!modal || !modal.dataset.propId) return;
  const p = adminData.properties.find(x => x.id === propId);
  if (!p) return;
  modal.innerHTML = _buildUnifiedModalHTML(p, modal.dataset.monthName, modal.dataset.year, 'propietario');
}
function ownerAddNote(propId) {
  const p = _getOwnerProp(propId); if (!p) return;
  const ta = document.getElementById('newOwnerNoteText');
  const txt = ta ? ta.value.trim() : '';
  if (!txt) { if(typeof toast==='function') toast('Escribe un texto antes de agregar','error'); return; }
  const today = new Date().toLocaleDateString('es-CO',{year:'numeric',month:'short',day:'numeric'});
  p.owner_notes.push({ date: today, text: txt });
  _refreshOwnerTab(propId);
}
function ownerRemoveNote(propId, idx) {
  const p = _getOwnerProp(propId); if (!p) return;
  p.owner_notes.splice(idx, 1);
  _refreshOwnerTab(propId);
}
function ownerAddImprovement(propId) {
  const p = _getOwnerProp(propId); if (!p) return;
  const title = prompt('Título de la mejora:');
  if (!title || !title.trim()) return;
  const description = prompt('Descripción (opcional):') || '';
  const price = prompt('Precio en pesos (opcional):') || '';
  p.improvements.push({ title: title.trim(), description, price, status: 'sugerida', photo_before_url: '', photo_after_url: '' });
  _refreshOwnerTab(propId);
}
function ownerRemoveImprovement(propId, idx) {
  const p = _getOwnerProp(propId); if (!p) return;
  if (!confirm('¿Eliminar esta mejora?')) return;
  p.improvements.splice(idx, 1);
  _refreshOwnerTab(propId);
}
function ownerChangeImprovStatus(propId, idx, newStatus) {
  const p = _getOwnerProp(propId); if (!p) return;
  if (p.improvements[idx]) p.improvements[idx].status = newStatus;
}
function ownerUploadImprovPhoto(propId, idx, side, input) {
  const p = _getOwnerProp(propId); if (!p) return;
  const file = input.files[0]; if (!file) return;
  if (file.size > 5 * 1024 * 1024) { if(typeof toast==='function') toast('Imagen demasiado grande (máx 5MB)','error'); return; }
  const reader = new FileReader();
  reader.onload = async (e) => {
    const base64 = e.target.result.split(',')[1];
    const mimeType = file.type;
    const btn = document.getElementById('btnSaveOwnerContent');
    if (btn) { btn.disabled = true; btn.textContent = '⏳ Subiendo foto...'; }
    try {
      const res = await fetch(ADMIN_SCRIPT_URL, {
        method: 'POST', headers: {'Content-Type':'application/json'},
        body: JSON.stringify({ action:'uploadOwnerPhoto', propertyId:propId, base64, mimeType, filename:file.name })
      });
      const data = await res.json();
      const url = data.fileUrl || e.target.result;
      if (side==='before') p.improvements[idx].photo_before_url = url;
      else                 p.improvements[idx].photo_after_url  = url;
      if(typeof toast==='function') toast(data.fileUrl?'Foto subida ✓':'Foto local (guarda para persistir)','success');
      _refreshOwnerTab(propId);
    } catch(err) {
      if (side==='before') p.improvements[idx].photo_before_url = e.target.result;
      else                 p.improvements[idx].photo_after_url  = e.target.result;
      if(typeof toast==='function') toast('Foto guardada localmente','info');
      _refreshOwnerTab(propId);
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = '💾 Guardar y Sincronizar'; }
    }
  };
  reader.readAsDataURL(file);
}
function ownerAddQuote(propId) {
  const p = _getOwnerProp(propId); if (!p) return;
  const item  = (document.getElementById('newQuoteItem')  ||{}).value||'';
  const desc  = (document.getElementById('newQuoteDesc')  ||{}).value||'';
  const val   = (document.getElementById('newQuoteValue') ||{}).value||'0';
  if (!item.trim()) { if(typeof toast==='function') toast('El campo Ítem es obligatorio','error'); return; }
  p.quotes.push({ item: item.trim(), description: desc.trim(), value: parseFloat(val)||0 });
  _refreshOwnerTab(propId);
}
function ownerRemoveQuote(propId, idx) {
  const p = _getOwnerProp(propId); if (!p) return;
  p.quotes.splice(idx, 1);
  _refreshOwnerTab(propId);
}
async function saveOwnerContent(propId) {
  const p = _getOwnerProp(propId); if (!p) return;
  const btn = document.getElementById('btnSaveOwnerContent');
  ...
  try {
    const res = await fetch(ADMIN_SCRIPT_URL, {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ action:'saveOwnerContent', propertyId:propId,
        owner_notes: p.owner_notes, improvements: p.improvements, quotes: p.quotes })
    });
    const data = await res.json();
    if (data.success) {
      if(typeof toast==='function') toast('✓ Contenido del propietario guardado','success');
      localStorage.setItem('icde_admin_data', JSON.stringify(adminData));
    } else {
      if(typeof toast==='function') toast('Error: '+(data.error||'desconocido'),'error');
    }
  } catch(e) {
    if(typeof toast==='function') toast('Error de red: '+e.message,'error');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '💾 Guardar y Sincronizar'; }
  }
}
...
/* ═══════════════════════════════════════════════════════════════════
   TESTIMONIOS — Panel de Administración (Parte A)
═══════════════════════════════════════════════════════════════════ */
let _adminTestimonials = [];   // cache local

async function renderAdminTestimonios(container) {
  container.innerHTML = `<div style="text-align:center;padding:40px;"><div class="spinner" style="margin:0 auto 16px;"></div><p style="color:var(--muted);">Cargando testimonios...</p></div>`;
  try {
    const res = await fetch(ADMIN_SCRIPT_URL + '?action=getTestimonials&t=' + Date.now());
    const data = await res.json();
    _adminTestimonials = Array.isArray(data.testimonials) ? data.testimonials : [];
  } catch(e) {
    _adminTestimonials = [];
  }
  _renderTestimonialsUI(container);
}

function _renderTestimonialsUI(container) {
  const listHTML = _adminTestimonials.length
    ? _adminTestimonials.map((t, i) => `
      <div style="background:var(--bg2);border:1px solid var(--border);border-radius:14px;padding:18px;display:flex;gap:14px;align-items:flex-start;position:relative;">
        <div style="width:44px;height:44px;border-radius:50%;background:rgba(212,168,75,0.15);border:1px solid rgba(212,168,75,0.3);display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;color:var(--gold);flex-shrink:0;">
          ${t.photo_url ? `<img src="${t.photo_url}" style="width:44px;height:44px;border-radius:50%;object-fit:cover;" />` : (t.display_name||'?').substring(0,2).toUpperCase()}
        </div>
        <div style="flex:1;min-width:0;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
            <div>
              <div style="font-size:13px;font-weight:700;color:#fff;">${t.display_name||'Propietario'}</div>
              ${t.property_context ? `<div style="font-size:11px;color:var(--muted);">${t.property_context}</div>` : ''}
            </div>
            <button onclick="deleteAdminTestimonial(${i})" style="background:none;border:none;color:rgba(239,68,68,0.5);cursor:pointer;font-size:13px;" title="Eliminar">&times;</button>
          </div>
          <div style="font-size:12px;color:rgba(255,255,255,0.7);margin-bottom:6px;border-left:2px solid rgba(239,68,68,0.4);padding-left:8px;"><strong style="color:#ef4444;">Problema:</strong> ${t.problem||''}</div>
          <div style="font-size:12px;color:rgba(255,255,255,0.7);border-left:2px solid rgba(34,197,94,0.4);padding-left:8px;"><strong style="color:#22c55e;">Solución:</strong> ${t.solution||''}</div>
        </div>
      </div>`).join('')
    : `<div style="text-align:center;padding:60px 20px;color:var(--muted);">
        <div style="font-size:40px;margin-bottom:12px;">💬</div>
        <p>No hay testimonios aún.</p>
        <p style="font-size:12px;margin-top:4px;">Agrega el primero usando el formulario de abajo.</p>
       </div>`;

  container.innerHTML = `
    <div style="max-width:780px;margin:0 auto;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
        <div style="font-size:16px;font-weight:800;color:var(--gold);">📋 Testimonios de Propietarios</div>
        <span style="font-size:12px;color:var(--muted);">${_adminTestimonials.length} registro(s)</span>
      </div>

      <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:28px;">${listHTML}</div>

      <div style="background:var(--bg2);border:1px solid rgba(212,168,75,0.2);border-radius:16px;padding:22px;">
        <div style="font-size:12px;color:var(--gold);font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:14px;">+ Agregar Testimonio</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
          <div>
            <label class="u-inp-label">Nombre / Iniciales (privacidad)</label>
            <input type="text" id="tsDisplayName" class="u-inp" placeholder="Ej: J.P.M o Juan P." />
          </div>
          <div>
            <label class="u-inp-label">Contexto inmueble (genérico)</label>
            <input type="text" id="tsContext" class="u-inp" placeholder="Ej: Apto norte de Neiva" />
          </div>
        </div>
        <div style="margin-bottom:12px;">
          <label class="u-inp-label">Foto / URL de imagen (opcional)</label>
          <input type="text" id="tsPhotoUrl" class="u-inp" placeholder="https://... (dejar vacío para usar iniciales)" />
        </div>
        <div style="margin-bottom:12px;">
          <label class="u-inp-label">Problema / Situación</label>
          <textarea id="tsProblem" class="u-inp" rows="2" style="resize:vertical;" placeholder="Describe el problema o situación que tenía el propietario..."></textarea>
        </div>
        <div style="margin-bottom:16px;">
          <label class="u-inp-label">Solución / Resultado</label>
          <textarea id="tsSolution" class="u-inp" rows="2" style="resize:vertical;" placeholder="Cómo lo resolvió ICDE y el resultado obtenido..."></textarea>
        </div>
        <button id="btnSaveTestimonial" onclick="saveAdminTestimonial()" class="btn btn-primary" style="width:100%;justify-content:center;font-weight:700;">
          💾 Guardar Testimonio
        </button>
      </div>
    </div>`;
...
async function saveAdminTestimonial() {
  const display_name    = (document.getElementById('tsDisplayName') ||{}).value||'';
  const property_context= (document.getElementById('tsContext')      ||{}).value||'';
  const photo_url       = (document.getElementById('tsPhotoUrl')     ||{}).value||'';
  const problem         = (document.getElementById('tsProblem')      ||{}).value||'';
  const solution        = (document.getElementById('tsSolution')     ||{}).value||'';
  if (!problem.trim() || !solution.trim()) {
    if(typeof toast==='function') toast('Completa al menos el Problema y la Solución','error');
    return;
  }
  const btn = document.getElementById('btnSaveTestimonial');
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Guardando...'; }
  try {
    const res = await fetch(ADMIN_SCRIPT_URL, {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ action:'saveTestimonial', display_name, property_context, photo_url, problem, solution })
    });
    const data = await res.json();
    if (data.success) {
      if(typeof toast==='function') toast('✓ Testimonio guardado','success');
      _adminTestimonials.push({ display_name, property_context, photo_url, problem, solution });
      const container = document.getElementById('adminTabContent');
      if (container) _renderTestimonialsUI(container);
    } else {
      if(typeof toast==='function') toast('Error: '+(data.error||'desconocido'),'error');
    }
  } catch(e) {
    ...
    if (btn) { btn.disabled = false; btn.textContent = '💾 Guardar Testimonio'; }
  }
}

async function deleteAdminTestimonial(idx) {
  if (!confirm('¿Eliminar este testimonio?')) return;
  const t = _adminTestimonials[idx];
  if (!t) return;
  try {
    await fetch(ADMIN_SCRIPT_URL, {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ action:'deleteTestimonial', id: t.id })
    });
  } catch(e) {}
  _adminTestimonials.splice(idx, 1);
  const container = document.getElementById('adminTabContent');
  if (container) _renderTestimonialsUI(container);
}
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (resData.success) {
        ...
        const monthsFullList = ["ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "JUNIO", "JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"];
        [currentYear - 1, currentYear, currentYear + 1].forEach(yr => {
          newProp.payments[String(yr)] = monthsFullList.map(m => ({
            month: m,
            value: tenant_name ? 'Pendiente' : 'DESOCUPADO',
            status: tenant_name ? 'PENDING' : 'VACANT'
          }));
        });
        syncPropertyContractMonths(newProp);
...
async function saveUnifiedAdminCobro(propId, propName, monthName, year) {
  ...
  if (status === 'PAID') {
    ...
    if (isNaN(parsedVal) || parsedVal <= 0) {
      const p = (typeof findAdminProperty === 'function') ? findAdminProperty(propId, propName) : adminData.properties.find(x => x.id === propId);
      const rentInfo = p ? calculateAdminRentInfo(p) : null;
      finalValue = (rentInfo && rentInfo.hasIncrease) ? rentInfo.increasedRent : (monthly_rent || (p ? p.monthly_rent : 0));
    } else { finalValue = parsedVal; }
  } else if (status === 'NEW_CONTRACT') {
    const parsedVal = parseFloat(rawValue);
    if (!isNaN(parsedVal) && parsedVal > 0) {
      ...
      const p = (typeof findAdminProperty === 'function') ? findAdminProperty(propId, propName) : adminData.properties.find(x => x.id === propId);
      const rentInfo = p ? calculateAdminRentInfo(p) : null;
      if (rentInfo && rentInfo.hasIncrease) {
        finalValue = rentInfo.increasedRent;
      } else {
        finalValue = 'CONTRATO NUEVO';
      }
    }
  ...
  saveBtn.disabled = true;
  saveBtn.innerHTML = '<span class="spinner" style="width:14px; height:14px; border-width:2px; display:inline-block; margin-right:6px; vertical-align:middle;"></span> Guardando en Google Drive...';
  ...
  const payloadPay = {
    ...
    year: year, monthIndex: monthIdx, value: finalValue, status: status
  };

  const resProp = await sendAdminCloudAction(payloadProp);
  const resPay = await sendAdminCloudAction(payloadPay);

  if ((resProp && resProp.success) || (resPay && resPay.success)) {
    toast('¡Cambios guardados directamente en Google Drive con éxito!', 'success');
  } else {
    toast('⚠ No se pudo confirmar el guardado en Google Drive', 'warning');
  }
...
async function sendAdminCloudAction(payload) {
  try {
    const res = await fetch(ADMIN_SCRIPT_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'text/plain' },
    ...
    console.warn('[CloudSync] POST directo falló:', e);
  }
  ...
  try {
    ...
    for (const k in payload) {
      if (payload[k] !== undefined && payload[k] !== null) {
        searchParams.append(k, String(payload[k]));
      }
    }
    const resGet = await fetch(ADMIN_SCRIPT_URL + '?' + searchParams.toString());
    if (resGet.ok) {
      const dataGet = await resGet.json();
      if (dataGet && dataGet.success) return dataGet;
    }
  } catch (e) {
    console.warn('[CloudSync] GET fallback falló:', e);
  }
...
async function saveAdminPaymentStatus(propId, propName, monthName, year) {
  ...
  if (status === 'PAID') {
    const parsedVal = parseFloat(rawValue);
    if (isNaN(parsedVal) || parsedVal <= 0) {
      const p = findAdminProperty(propId, propName);
  ...
  const res = await sendAdminCloudAction(payload);
  if (res && res.success) {
    toast('¡Pago actualizado directamente en Google Drive con éxito!', 'success');
  } else {
    toast('⚠ No se pudo actualizar el pago en Google Drive', 'warning');
  }
...
async function quickSaveAdminPaymentStatus(propId, propName, monthName, year, status) {
  ...
  // Actualización optimista de memoria temporal
  const p = findAdminProperty(propId, propName);
  if (p) {
  ...
  const res = await sendAdminCloudAction(payload);
  if (res && res.success) {
    toast('¡Pago actualizado directamente en Google Drive!', 'success');
  } else {
    toast('⚠ No se pudo sincronizar con Google Drive', 'warning');
  }
  await loadAdminData();
  renderAdministracion();
}
...
async function saveAdminPropertyDetails(propId, propNameOld) {
  ...
  const res = await sendAdminCloudAction(payload);
  if (res && res.success) {
    toast('¡Inmueble guardado directamente en Google Drive con éxito!', 'success');
  } else {
    toast('⚠ No se pudo confirmar el guardado en Google Drive', 'warning');
  }
  await loadAdminData();
...
async function saveAdminContractDetails(propId, propNameOld) {
  ...
  const p = findAdminProperty(propId, propNameOld);
  ...
  const res = await sendAdminCloudAction(payload);
  if (res && res.success) {
    toast('¡Contrato actualizado en Google Drive con éxito!', 'success');
  } else {
    toast('⚠ No se pudo actualizar el contrato en Google Drive', 'warning');
  }
  await loadAdminData();
  closeUnifiedModal();
  renderAdministracion();
...
function getStatusLabel(status) {
  switch(status) {
    ...
    case 'NEW_CONTRACT': return 'Renovación / Término de Contrato';
...
async function refreshAdminData() {
  toast('🔄 Leyendo datos reales directamente de Google Drive...', 'info');
  await loadAdminData(true);
  renderAdministracion();
}

async function reloadDirectFromDrive() {
  await refreshAdminData();
}

async function syncAllAdminLocalPaymentsToCloud(btn) {
  if (!adminData || !adminData.properties || !adminData.properties.length) {
    toast('No hay datos locales para sincronizar.', 'warning');
    return;
  }
  if (!confirm('¿Deseas subir TODOS los datos y pagos de administración guardados en este computador a la nube en Google Sheets?')) return;
  ...
  toast('Sincronizando pagos con la nube en Google Sheets...', 'info');

  let totalPushed = 0;
  for (const p of adminData.properties) {
    const payloadProp = {
      action: 'saveAdminProperty',
      propertyId: p.id,
      name: p.name,
      ...
      duration: p.duration || '',
      due_day: p.due_day || 5,
      max_due_day: p.max_due_day || 10,
      increase_notes: p.increase_notes || '',
      damage_notes: p.damage_notes || ''
    };
    await sendAdminCloudAction(payloadProp);

    if (p.payments) {
      for (const yearStr of Object.keys(p.payments)) {
        const yearNum = parseInt(yearStr, 10);
        const mList = p.payments[yearStr];
        if (Array.isArray(mList)) {
          for (let mIdx = 0; mIdx < mList.length; mIdx++) {
            const payItem = mList[mIdx];
            if (payItem && payItem.value !== undefined && payItem.value !== '' && payItem.value !== '-') {
              const payloadPay = {
                action: 'saveAdminPayment',
                propertyId: p.id,
                propertyName: p.name,
              ...
              await sendAdminCloudAction(payloadPay);
              totalPushed++;
            }
          }
        }
  ...
  toast(`✅ ¡${totalPushed} registros subidos a la nube en Google Sheets!`, 'success');
  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '☁️ Subir Pagos a la Nube';
  }
}
