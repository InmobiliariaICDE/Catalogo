...
window.lbAbrir = function(fotos, idx){
  ...
  const lbImgWrap = document.querySelector('#lightboxOverlay .lightbox-img-wrap');
  if (lbImgWrap && !lbImgWrap._swipeInit) {
    lbImgWrap._swipeInit = true;
    let _lbTx = null, _lbTy = null, _lbMoved = false;
    ...
    lbImgWrap.addEventListener('touchmove', function(e){
      ...
      if (Math.abs(dx) > 10 && Math.abs(dx) > Math.abs(dy)) _lbMoved = true;
    }, { passive: true });
    ...
    lbImgWrap.addEventListener('pointercancel', () => { pDown = false; });
  }
...
let mpCarruselTimer = null;
const MP_CARRUSEL_DELAY = 5000;

function mpCarruselReiniciarProgreso() {
  if (!mpCarruselFotos || mpCarruselFotos.length <= 1) return;
  clearTimeout(mpCarruselTimer);
  mpCarruselTimer = setTimeout(() => mpCarruselIr(1), MP_CARRUSEL_DELAY);
}

function mpCarruselDetener() {
  clearTimeout(mpCarruselTimer);
}

function mpCarruselActualizarMiniaturas() {
  const n = mpCarruselFotos.length;
  if (!n) return;
  const minDiv = document.getElementById('mpCarruselMiniaturas');
  if (!minDiv) return;

  if (n <= 4) {
    minDiv.innerHTML = mpCarruselFotos.map((u, i) => 
      `<img src="${u}" class="carrusel-min ${i === mpCarruselIdx ? 'activa' : ''}" onclick="mpCarruselIrA(${i})"/>`
    ).join('');
  } else {
    let start = Math.max(0, mpCarruselIdx - 1);
    if (start + 4 > n) start = Math.max(0, n - 4);
    const ventana = [];
    for (let i = start; i < start + 4 && i < n; i++) ventana.push(i);
    minDiv.innerHTML = ventana.map(i => 
      `<img src="${mpCarruselFotos[i]}" class="carrusel-min ${i === mpCarruselIdx ? 'activa' : ''}" onclick="mpCarruselIrA(${i})"/>`
    ).join('');
  }
}
...
function abrirModalProp(codigoOrProp){
  ...
  tags.innerHTML = '';
  if(p['Habitaciones']) tags.innerHTML += `<div class="modal-prop-tag"><img src="https://i.imgur.com/ykKdGwE.png" width="13"/> Habitaciones <strong>${p['Habitaciones']}</strong></div>`;
  if(p['Baños']) tags.innerHTML += `<div class="modal-prop-tag"><img src="https://i.imgur.com/h9NqA32.png" width="13"/> Baños <strong>${p['Baños']}</strong></div>`;
  if(p['Garaje'] && p['Garaje']!=='No') tags.innerHTML += `<div class="modal-prop-tag"><img src="https://i.imgur.com/4Yixa77.png" width="13"/> Garaje <strong>${p['Garaje']}</strong></div>`;
  if(p['Cocina'] && p['Cocina']!=='No') tags.innerHTML += `<div class="modal-prop-tag"><img src="https://i.imgur.com/rH6cXMa.png" width="13"/> Cocina <strong>${p['Cocina']}</strong></div>`;
  if(p['Pisos']) tags.innerHTML += `<div class="modal-prop-tag"><img src="https://img.icons8.com/ios-filled/50/ffffff/stairs.png" width="13"/> Pisos <strong>${p['Pisos']}</strong></div>`;
  const areaLote = p['Área lote'] || p['Área Lote'] || p['Area lote'];
  if(areaLote) tags.innerHTML += `<div class="modal-prop-tag"><img src="https://i.imgur.com/rz72lGC.png" width="13"/> Lote <strong>${areaLote}${String(areaLote).toLowerCase().includes('m²') ? '' : ' m²'}</strong></div>`;
...
function mpCarruselInit(){
  ...
  if (principal && !principal._swipeInit) {
    principal._swipeInit = true;
    let _tSx = 0, _tSy = 0, _tMoved = false;
    principal.addEventListener('touchstart', function(e){
      if (e.touches.length !== 1) return;
      _tSx = e.touches[0].clientX; _tSy = e.touches[0].clientY; _tMoved = false;
    }, { passive: true });
    principal.addEventListener('touchmove', function(e){
      if (Math.abs(e.touches[0].clientX - _tSx) > 10) _tMoved = true;
    }, { passive: true });
    principal.addEventListener('touchend', function(e){
      if (!_tMoved) return;
      const dx = e.changedTouches[0].clientX - _tSx;
      const dy = e.changedTouches[0].clientY - _tSy;
      _tMoved = false;
      if (Math.abs(dx) < 30 || Math.abs(dy) > Math.abs(dx)) return;
      if (dx < 0) mpCarruselIr(1); else mpCarruselIr(-1);
    }, { passive: true });

    let pDown = false, pSx = 0;
    principal.addEventListener('pointerdown', function(e){
      if (e.button !== 0) return;
      pDown = true; pSx = e.clientX;
    });
    principal.addEventListener('pointerup', function(e){
      if (!pDown) return;
      const dx = e.clientX - pSx;
      pDown = false;
      if (Math.abs(dx) > 35) {
    ...
    principal.addEventListener('pointercancel', () => { pDown = false; });
  }
  
  mpCarruselActualizarMiniaturas();
  document.getElementById('mpCarruselCounter').textContent = `1 / ${mpCarruselFotos.length}`;
  mpCarruselReiniciarProgreso();
}
...
function mpCarruselIrA(idx, reiniciarTimer = true){
  const n = mpCarruselFotos.length;
  if(n<=1) return;
  ...
  if (reiniciarTimer) mpCarruselReiniciarProgreso();
}
...
function contRenderHero(){
  ...
  el.innerHTML=
    '<div class="cont-hero-card" style="--card-accent:#22c55e;"><div class="ch-top-row"><span class="ch-icon">\uD83D\uDCB9</span><div class="ch-lbl">Ingresos Totales</div></div><div class="ch-val">'+contFmt(ingresos)+'</div><div class="ch-sub">'+lista.filter(m=>m.tipo==='ingreso').length+' transacciones</div></div>'+
    '<div class="cont-hero-card" style="--card-accent:#ef4444;"><div class="ch-top-row"><span class="ch-icon">\uD83D\uDCC9</span><div class="ch-lbl">Egresos Totales</div></div><div class="ch-val">'+contFmt(egresos)+'</div><div class="ch-sub">'+lista.filter(m=>m.tipo==='egreso').length+' transacciones</div></div>'+
    '<div class="cont-hero-card" style="--card-accent:'+(utilidad>=0?'#22c55e':'#ef4444')+'"><div class="ch-top-row"><span class="ch-icon">'+(utilidad>=0?'\uD83D\uDCC8':'\uD83D\uDCC9')+'</span><div class="ch-lbl">Utilidad Neta</div></div><div class="ch-val" style="color:'+(utilidad>=0?'#22c55e':'#ef4444')+'">'+contFmt(utilidad)+'</div><div class="ch-sub">Margen: '+margen+'%</div></div>'+
    '<div class="cont-hero-card" style="--card-accent:var(--gold)"><div class="ch-top-row"><span class="ch-icon">\uD83D\uDCCA</span><div class="ch-lbl">Margen</div></div><div class="ch-val" style="color:var(--gold)">'+margen+'%</div><div class="ch-sub">'+(utilidad>=0?'\u2705 Positivo':'\u26A0\uFE0F Negativo')+'</div></div>'+
    '<div class="cont-hero-card" style="--card-accent:#3b82f6"><div class="ch-top-row"><span class="ch-icon">\uD83C\uDFAF</span><div class="ch-lbl">Meta Mes</div></div><div class="ch-val" style="color:#3b82f6">'+pMeta+'%</div><div class="ch-sub">Meta: '+contFmt(contMetas.ingMes)+'</div></div>';
}
...
function contSeleccionarMesYVerMovimientos(mes){
  ...
  } else if (contDetalleMesSortOrder === 'ultimo') {
    listaMes.sort(function(a, b) {
      ...
      const timeA = a.isAuto ? 0 : (latestA ? new Date(latestA.fechaAccion).getTime() : (a.creadoEn ? new Date(a.creadoEn).getTime() : (a.fecha ? new Date(a.fecha + 'T00:00:00').getTime() : 0)));
      ...
      const timeB = b.isAuto ? 0 : (latestB ? new Date(latestB.fechaAccion).getTime() : (b.creadoEn ? new Date(b.creadoEn).getTime() : (b.fecha ? new Date(b.fecha + 'T00:00:00').getTime() : 0)));
    ...
    body.innerHTML = 
      ...
      '<thead>' +
        '<tr style="border-bottom:1px solid rgba(255,255,255,0.08);">' +
          '<th style="text-align:left; color:#D4A84B; background:transparent; font-size:11px; text-transform:uppercase;">Fecha</th>' +
          '<th style="text-align:left; color:#D4A84B; background:transparent; font-size:11px; text-transform:uppercase;">Categoría</th>' +
          '<th style="text-align:left; color:#D4A84B; background:transparent; font-size:11px; text-transform:uppercase;">Descripción</th>' +
          '<th style="text-align:right; color:#D4A84B; background:transparent; font-size:11px; text-transform:uppercase;">Monto</th>' +
          '<th style="text-align:center; color:#D4A84B; background:transparent; font-size:11px; text-transform:uppercase;">Acc.</th>' +
        '</tr>' +
      '</thead>' +
      '<tbody>' +
...
function contRenderFlujo(el){
  ...
  '</div>'+
  '<div class="panel-section cont-header-panel-section" style="padding: 4px 4px; background: rgba(255,255,255,0.04); border: none; border-radius: 16px; box-shadow: 3px 3px 6px rgba(0,0,0,0.4), inset 1px 1px 0px rgba(255,255,255,0.1), inset -1px -1px 0px rgba(0,0,0,0.3); margin: 4px 0px 4px;">'+
    '<div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; width:100%;">'+
      '<div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">'+
        '<span style="font-size:13px; font-weight:700; color:var(--gold); letter-spacing:0.05em;">💰 Contabilidad:</span>'+
        '<div class="btn-nueva-cita-premium" style="height:38px; font-size:12.5px; padding:0 10px; border-radius:8px; display:inline-flex; align-items:center; justify-content:center; gap:4px;">'+
          '<button onclick="contCambiarAno(-1)" style="background:none; border:none; color:var(--gold); cursor:pointer; font-size:12px; padding:0 4px; display:inline-flex; align-items:center; transition:transform 0.15s;" onmouseover="this.style.transform=\'scale(1.25)\'" onmouseout="this.style.transform=\'scale(1)\'">◄</button>'+
          '<span style="font-size:13px; font-weight:700; color:#fff; min-width:36px; text-align:center;">'+contAnoFiltro+'</span>'+
          '<button onclick="contCambiarAno(1)" style="background:none; border:none; color:var(--gold); cursor:pointer; font-size:12px; padding:0 4px; display:inline-flex; align-items:center; transition:transform 0.15s;" onmouseover="this.style.transform=\'scale(1.25)\'" onmouseout="this.style.transform=\'scale(1)\'">►</button>'+
        '</div>'+
        '<select id="contMesSel" class="btn-nueva-cita-premium" style="width:auto; height:38px; padding:0 14px; font-size:13px; border-radius:8px; cursor:pointer; color-scheme:dark; font-family:\'Outfit\',sans-serif; color:#fff;" onchange="contCambiarMes(this.value)">'+
          '<option value="0" style="background:#18181b; color:#fff;">Todos los meses</option>'+
          CONT_MESES_FULL.map(function(m,i){return'<option value="'+(i+1)+'"'+(contMesFiltro===(i+1)?' selected':'')+' style="background:#18181b; color:#fff;">'+m+'</option>';}).join('')+
        '</select>'+
      '</div>'+
      '<div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">'+
        '<button class="btn-nueva-cita-premium" onclick="contAbrirMetas()" style="height:38px; font-size:13px; padding:0 14px; border-radius:8px; display:inline-flex; align-items:center; gap:6px; margin:0;">🎯 Metas</button>'+
        '<button class="btn-nueva-cita-premium" onclick="contSwitchTab(\'inversiones\')" style="height:38px; font-size:13px; padding:0 14px; border-radius:8px; display:inline-flex; align-items:center; gap:6px; margin:0;">💎 Inversiones</button>'+
        '<button class="btn-nueva-cita-premium" onclick="contToggleGraficas()" style="height:38px; font-size:13px; padding:0 14px; border-radius:8px; display:inline-flex; align-items:center; gap:6px; margin:0;">📊 Gráficas</button>'+
      '</div>'+
    '</div>'+
  '</div>'+
  '<div class="cont-panel panel-section" style="padding:0; overflow:hidden; background: rgba(255,255,255,0.04); border: none; border-radius: 16px; box-shadow: 3px 3px 6px rgba(0,0,0,0.4), inset 1px 1px 0px rgba(255,255,255,0.1), inset -1px -1px 0px rgba(0,0,0,0.3);">'+
  '<div class="cont-panel-body" style="padding:0;"><table style="width:100%;border-collapse:collapse;font-size:13px;">'+
  '<thead><tr style="border-bottom:1px solid rgba(255,255,255,0.08);"><th style="text-align:left;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Mes</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Ingresos</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Egresos</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Utilidad</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Margen</th></tr></thead>'+
...
function contRenderPareto(el){
  ...
  (totalIng===0?'<div class="cont-empty">Sin ingresos para analizar.</div>':
    ...
    '<table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:16px;"><thead><tr style="border-bottom:1px solid rgba(255,255,255,0.08);"><th style="text-align:left;color:#D4A84B;background:transparent;font-size:10px;text-transform:uppercase;">Categor\u00EDa</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:10px;text-transform:uppercase;">Ingresos</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:10px;text-transform:uppercase;">%</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:10px;text-transform:uppercase;">% Acum.</th><th style="text-align:center;color:#D4A84B;background:transparent;font-size:10px;text-transform:uppercase;">Impacto</th></tr></thead>'+
  ...
  <div style="background:linear-gradient(135deg,rgba(212,168,75,0.08),rgba(34,197,94,0.04));border:1px solid rgba(212,168,75,0.2);border-radius:14px;padding:18px 22px;margin-bottom:20px;">
    <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap;justify-content:space-between;">
      ...
      <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
        <button onclick="contSwitchTab('flujo')" style="background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); color:#fff; font-size:12px; font-weight:700; font-family:'Outfit', sans-serif; padding:6px 14px; border-radius:20px; cursor:pointer; display:inline-flex; align-items:center; gap:5px; box-shadow:0 2px 6px rgba(0,0,0,0.3); transition:all 0.15s ease;" onmouseover="this.style.background='rgba(212,168,75,0.15)'; this.style.borderColor='var(--gold)'; this.style.color='var(--gold)';" onmouseout="this.style.background='rgba(255,255,255,0.06)'; this.style.borderColor='rgba(255,255,255,0.12)'; this.style.color='#fff';">📈 Flujo Mensual</button>
...
function contRenderMovimientos(el){
  ...
  '<table style="width:100%;border-collapse:collapse;font-size:13px;">'+
  '<thead><tr style="border-bottom:1px solid rgba(255,255,255,0.08);"><th style="text-align:left;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Fecha</th><th style="text-align:left;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Categor\u00EDa</th><th style="text-align:left;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Descripci\u00F3n</th><th style="text-align:right;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Monto</th><th style="text-align:center;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Tipo</th><th style="text-align:center;color:#D4A84B;background:transparent;font-size:11px;text-transform:uppercase;">Acc.</th></tr></thead>'+
...
function contToggleTipo(){
  ...
  const ingresosCats=['Venta de Inmueble','Separan Inmueble','Arriendo','Aval\u00FAos','Remodelaci\u00F3n','Reparaci\u00F3n','Arquitectura','Gesti\u00F3n/Administraci\u00F3n','Consultor\u00EDa','Recaudo de cartera','Otro Servicio'];
...
function contGuardarMovimiento(){
  ...
  movementsToAdd.forEach(function(obj) {
    ...
    if(idx>=0) {
      ...
      if (oldMov.creadoEn) obj.creadoEn = oldMov.creadoEn;
